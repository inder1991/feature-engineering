...
#: The V2 canonical hash of `posted_debit_amount` BEFORE A6's G2 ruling — recorded, not deleted,
#: so the regeneration below stays checkable and a silent revert of the ruling fails loudly.
_PRE_A6_POSTED_DEBIT_AMOUNT_V2_HASH = (
    "37b37069833163063a90c88a77ec2615107392a281bcb3906fbdc47d2c467b34")


def test_the_registry_bypass_is_identity_neutral():
    """The legacy literal below was computed on the PRE-change checkout (commit 219d8360, clean
    tree). It pins the canonicalization a new `Template`/`Need` FIELD would have moved: adding a
    field must not move a recipe's identity, and recomputing both sides with the same post-change
    code would only prove self-agreement.

    ▲ **The V2 literal was REGENERATED by A6** (fix round), and the distinction matters. This pin
    catches a *field addition* moving identity silently; A6 did not add a field, it changed a
    DECLARED VALUE — `posted_debit_amount`'s `transaction` and `original_txn` operands now declare
    `join_role="intermediate_entity_key"`, the G2 ruling for those two slots
    (`recipes/transaction_foundation.py`). `canonical_recipe_v2` is field-exhaustive by
    construction, so a changed declaration MUST move the hash: that is the mechanism working, not
    a regression. What the pin still proves is unchanged — at a fixed recipe content, neither
    canonicalization drifts — and the pre-value is asserted DIFFERENT so the ruling cannot be
    reverted without this test going red.
    """
    assert ALL_TEMPLATES[0].id == "balance_trend"
    assert recipe_content_hash(ALL_TEMPLATES[0]) == "rh_4bc9a3f80f885743"
    current = canonical_recipe_v2_hash(v2_recipe_by_id("posted_debit_amount"))
    assert current == "331cb8525546cffedadb408edef492ea1625c5d39df5b4c0da57204a9b3ff120"
    assert current != _PRE_A6_POSTED_DEBIT_AMOUNT_V2_HASH


def test_the_g2_ruling_did_not_unship_the_only_reviewed_v2_expectation():
    """A6's must-resolve-first gate, kept as a permanent guard. The reviewed Formula-v2
    expectation is keyed on the `expectation_ref` STRING (`recipe_formula_expectations_v2`
    docstring: "The key is an EXPECTATION REF, never a recipe id"), and the gold fixture's pin is
    the fixture's own canonical bytes — so moving the recipe's content hash cannot strip it. If a
    future change ever keys review on the recipe hash, this fails and the exemplar's reviewed
    status is protected by a test rather than by an argument in a report."""
    ...
    assert recipe.revision == 1, "the ruling needed no revision bump — the hash IS the identity"
    assert recipe.formula.expectation_ref == "posted_debit_amount"
    assert has_reviewed_expectation(recipe.formula.expectation_ref)
    assert recipe.readiness == "FORMULA_AUTHORABLE"
