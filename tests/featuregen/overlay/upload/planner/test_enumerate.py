...
def test_single_eligible_per_need_yields_one_resolved_plan():
    ...
    # ingredient_binding_only (the B5 assembler enriches it); candidate_role is `unranked` —
    # only the cross-catalog rank_and_classify assigns selected/alternative/rejected, and it
    # never processes tier-1 plans (order_plans doesn't touch candidate_role).
    ...
    assert p.candidate_role is CandidateRole.unranked
