...
def test_owned_discovery_coverage_gate_passes_non_vacuously() -> None:
    ...
    # ...and the debt is REPORTED, not laundered: most of the registry's applicability is still
    # legacy-derived (the number Task 17 drives to zero), and nothing is executable-covered.
    assert result["legacy_derived_recipe_count"] > 100
...
def test_nothing_is_executable_covered_until_the_gold_gate_is_proven() -> None:
    """The BR-9 acceptance, pinned as of today: the two reviewed expectation anchors rest at
    FORMULA_AUTHORABLE (BR-7's fold — gold unproven), which is deliberately NOT executable."""
    report = coverage_report()
    ...
    assert set(readiness.values()) == {"UNASSESSED", "FORMULA_AUTHORABLE"}
...
def test_every_gap_is_scheduled_and_every_empty_leaf_is_documented() -> None:
    ...
    assert all(row["target_increment"].startswith("BR-1") for row in targets["backlog"])
