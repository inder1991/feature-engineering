def calculate_total_debit_amount_30d(
    ...
    aggregate = F.countDistinct(F.col('txn_amt'))

    # The grain-level aggregate. `count_distinct` is the expression's own declared aggregate, and
    # the grouping is the DECLARED grain — one row per landing key, which is what the spine
    # reduction below can then join onto exactly once.
    grouped = rows.groupBy(F.col('cif_id')).agg(aggregate.alias('total_debit_amount_30d'))
    ...
    # §6 — the published type is BIGINT, which is integral. Neither of §6's decimal obligations
    # applies to it: there is no decimal arithmetic for a rounding mode to govern, and no decimal
    # overflow to check for. A rounding mode recorded against it would be an instruction this node
    ...
    schema_hash = hashlib.sha256(json.dumps(
        {'columns': staged.columns,
         'feature': ['total_debit_amount_30d', 'BIGINT', False]},
