def calculate_cross_border_value_ratio_90d(
    ...
    declared empty-window value (body.numerator: null, body.denominator: zero). An inner join
    ...
    `zero`. A denominator of ZERO and an ABSENT denominator are different facts that both look
    ...
    # §8 rule 4 — the declared null_input is `propagate`, so ONE null txn_amt anywhere in the
    # group makes the whole aggregate NULL. Spark's aggregates skip nulls, so this is rendered
    # explicitly; it is not what any of them does on its own.
    numerator_any_null = F.count(F.col('txn_amt')) < F.count(F.lit(1))
    numerator_aggregate = F.when(numerator_any_null, F.lit(None).cast('decimal(38,6)')).otherwise(
        F.sum(F.col('txn_amt')))

    # The operand count rides along for §9's overflow gate below, and under `propagate` it is
    # ZEROED for every group the policy itself answers: one null txn_amt makes the aggregate NULL
    # by declaration, and that NULL must never read as an overflow. What remains — a NULL sum
    ...
    # it.
    numerator_operand_count = F.when(numerator_any_null, F.lit(0).cast('bigint')).otherwise(
        F.count(F.col('txn_amt')))
    ...
    denominator_grouped = denominator_rows.groupBy(F.col('cif_id')).agg(
        ...
        F.count(F.lit(1)).alias('__denominator_source_rows'),
    )
    ...
    # §8 rule 4 — `body.denominator`'s declared empty_window is `zero`. The marker is what
    # distinguishes an entity with NO rows from one whose aggregate evaluated to NULL: both are
    # null after the join, and only the first is an empty window. Coalescing instead would answer
    # the second one's policy question with this one's answer.
    denominator_no_source_rows = F.col('__denominator_source_rows').isNull()
    denominator_empty_value = F.lit(0).cast('decimal(38,6)')
    staged = staged.withColumn(
        '__denominator',
        F.when(denominator_no_source_rows, denominator_empty_value).otherwise(
            F.col('__denominator')))
    ...
    staged = staged.drop('__denominator_source_rows')
    ...
    zero_value = F.lit(0).cast('decimal(38,6)')
    staged = staged.withColumn(
