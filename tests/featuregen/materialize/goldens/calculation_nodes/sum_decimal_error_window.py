def calculate_total_debit_amount_30d(
    ...
    source rows is still present and carries this formula's declared empty-window value (error).
    ...
    # §8 rule 4 — the declared empty_window is `error`, so an entity with no rows in the window
    # STOPS the run. The marker is what makes that decidable: after the join an empty window and a
    # null aggregate look identical, and only one of them is what the formula declared an error.
    # This is the FORMULA's policy, not a §9 validation gate, so it deliberately carries no
    # ValidationGateCode: §14's vocabulary has no member for it, and borrowing one would put this
    # decision into the gate table's mouth.
    no_source_rows = F.col('__source_rows').isNull()
    empty = staged.where(no_source_rows)
    if empty.limit(1).count() > 0:
        raise RuntimeError(
            "total_debit_amount_30d declares empty_window=error, and at least one entity in the "
            "declared population has no source row in its window. The formula asks for the run to "
            "stop rather than for a value to be invented for it. Entities affected: "
            + str(empty.count()))
