def calculate_cross_border_value_ratio_90d(
    ...
    `error`. A denominator of ZERO and an ABSENT denominator are different facts that both look
    ...
    # would otherwise publish a column of nulls and report nothing. It is the FORMULA's policy and
    # not a §9 validation gate, so it carries no ValidationGateCode — §14's vocabulary has no
    # member for it.
    divided_by_zero = staged.where(denominator_is_zero)
    if divided_by_zero.limit(1).count() > 0:
        raise RuntimeError(
            "cross_border_value_ratio_90d declares zero_denominator=error, and at least one "
    ...
    # they still exist. The `error` zero_denominator policy proved above that no denominator
    # reaching the division is zero, so it needs no exclusion here.
