def project_total_debit_amount_30d__body_expr(
        source: DataFrame,
        join_1_accounts: DataFrame,
        join_2_customers: DataFrame,
    ...
    rows = source.select(
        F.col('acct_num'), F.col('dr_cr_flag'), F.col('posted_ts'), F.col('status_cd'),
    ...
    # §3.1 — the governed traversal to the grain: 2 hops, each one an edge the join planner
    ...
    hop_1_joinable = hop_1.where(F.col('__join_1__key').isNotNull())
    duplicate_1 = hop_1_joinable.groupBy(F.col('__join_1__key')).count().where(
    ...
    # Hop 2: banking.accounts.owner_id -> banking.customers.cif_id, cardinality N:1 toward
    ...
    hop_2_joinable = hop_2.where(F.col('__join_2__key').isNotNull())
    duplicate_2 = hop_2_joinable.groupBy(F.col('__join_2__key')).count().where(
        F.col('count') > F.lit(1)).limit(1).count()
    if duplicate_2:
        raise RuntimeError(
            'JOIN_AMPLIFICATION' +
    ...
    # way.
    rows = rows.select(
        F.col('acct_num'), F.col('__join_2__cif_id').alias('cif_id'), F.col('dr_cr_flag'),
