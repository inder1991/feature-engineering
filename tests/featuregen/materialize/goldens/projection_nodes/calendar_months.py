def project_total_debit_amount_30d__body_expr(source: DataFrame, business_dt: str) -> DataFrame:
    ...
    # A calendar period: the window ends where the CURRENT month begins, so
    # the incomplete month the business date sits in is outside it. That is
    # the whole difference from a trailing window, and it is a different set of rows.
    window_end = F.trunc(anchor, 'month')
    # 3 month(s) back, as CALENDAR months: `add_months`
