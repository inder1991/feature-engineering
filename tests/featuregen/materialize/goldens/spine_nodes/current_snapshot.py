def build_spine(source: DataFrame, business_dt: str) -> DataFrame:
    ...
    if business_date != '2026-07-27':
        raise RuntimeError(
            'SPINE_INCOMPLETE: ' + "the declared population is a snapshot observed at "
            + '2026-07-27' + " and this run asks for " + repr(business_date)
            + ": a table that holds no history cannot honestly answer another business "
            "date, so the spine refuses rather than answering with the rows it holds today")
    rows = source
