...
def test_accepted_when_on_target(conn):
    ...
    out = accept_result(
        conn, cid, current_run_id="run_1", current_stream_version=5, current_task_id=None
    )
...
def test_stale_when_run_changed(conn):
    ...
    out = accept_result(
        conn, cid, current_run_id="run_2", current_stream_version=5, current_task_id=None
    )
...
def test_stale_when_advanced_past_version(conn):
    ...
    out = accept_result(
        conn, cid, current_run_id="run_1", current_stream_version=6, current_task_id=None
    )
...
def test_stale_when_task_changed(conn):
    ...
    out = accept_result(
        conn, cid, current_run_id=None, current_stream_version=None, current_task_id="task_2"
    )
    assert out.stale is True
...
def test_stale_idempotent_cache(conn):
    ...
    accept_result(
        conn, cid, current_run_id="run_2", current_stream_version=None, current_task_id=None
    )
    out = accept_result(
        conn, cid, current_run_id="run_2", current_stream_version=None, current_task_id=None
    )
...
def test_applied_result_is_cached(conn, insert_stub_event):
    ...
    with conn.cursor() as cur:
        cur.execute(
            "UPDATE external_commands SET result_event_id='evt_res' WHERE command_id=%s", (cid,)
        )
    out = accept_result(
        conn, cid, current_run_id="run_1", current_stream_version=5, current_task_id=None
    )
