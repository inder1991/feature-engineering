...
def test_first_activation_from_null_base_succeeds(db):
    ...
    row = db.execute(
        ...
        "WHERE feature_id='feat_a' AND use_case='fraud'"
    ).fetchone()
...
def test_use_case_scoped_coexistence(db):
    ...
    rows = dict(
        db.execute(
            "SELECT use_case, feature_version_id FROM feature_active_versions "
            "WHERE feature_id='feat_d'"
        ).fetchall()
    )
...
def test_request_activation_enqueues_feature_partition_and_appends_run_event(db):
    ...
    mid = request_activation(
        ...
        base_feature_version_id=None,
        approval_type="PRODUCTION",
        produced_by_run="run1",
        actor=make_actor(),
    )
    row = db.execute(
        "SELECT partition_key, handler, payload FROM queue WHERE message_id=%s", (mid,)
    ).fetchone()
...
def test_cas_claim_slot_first_writer_wins_no_silent_overwrite(db):
    ...
    row = db.execute(
        "SELECT feature_version_id FROM feature_active_versions "
        "WHERE feature_id='feat_cas' AND use_case='fraud'"
    ).fetchone()
...
def test_saga_step1_mints_version_and_enqueues_in_one_tx(db):
    res = on_run_approved(
        db,
        feature_id="feat_saga",
        produced_by_run="run_appr",
        use_case="fraud",
        approval_type="PRODUCTION",
        actor=make_actor(),
        provenance=provenance_for(),
    ...
    assert (
        db.execute(
            "SELECT count(*) FROM feature_versions WHERE feature_version_id=%s",
            (res.feature_version_id,),
        ).fetchone()[0]
        == 1
    )
    ...
    q = db.execute(
        "SELECT partition_key, handler FROM queue WHERE message_id=%s", (res.activation_message_id,)
    ).fetchone()
...
def test_activate_version_handler_executes_feature_side_activation(db):
    ...
    assert (
        db.execute(
            "SELECT count(*) FROM feature_active_versions WHERE feature_id='feat_hdl'"
        ).fetchone()[0]
        == 0
    )
    ...
    commit_step(
        ...
        result,
        message_id="msg_hdl",
        expected_version=current_version(db, "run", "run_h"),
        table_version=1,
    )
    row = db.execute(
        "SELECT feature_version_id, activation_state FROM feature_active_versions "
        "WHERE feature_id='feat_hdl' AND use_case='fraud'"
    ).fetchone()
...
def test_activate_version_handler_is_idempotent(db):
    ...
    request_activation(
        db,
        feature_id="feat_hdl2",
        feature_version_id=v1,
        use_case="fraud",
        base_feature_version_id=None,
        approval_type="PRODUCTION",
        produced_by_run="run_h2",
        actor=make_actor(),
    )
    ...
    commit_step(
        ...
        message_id="msg_h2a",
        expected_version=current_version(db, "run", "run_h2"),
        table_version=1,
    )
    commit_step(
        db,
    ...
    activations = [
        e for e in load_stream(db, "feature", "feat_hdl2") if e.type == "VERSION_ACTIVATED"
    ]
...
def test_activation_is_atomic_with_step_rollback(db):
    ...
    request_activation(
        db,
        feature_id="feat_atom",
        feature_version_id=v1,
        use_case="fraud",
        base_feature_version_id=None,
        approval_type="EXPERIMENTAL",
        produced_by_run="run_atom",
        actor=make_actor(),
        expires_at=exp,
    )
    ...
    try:
        with db.transaction():  # mirrors process_one's per-step savepoint
            commit_step(
                ...
                result,
                message_id="msg_atom",
                expected_version=current_version(db, "run", "run_atom"),
                table_version=1,
            )
    ...
    assert (
        db.execute(
            "SELECT count(*) FROM feature_active_versions WHERE feature_id='feat_atom'"
        ).fetchone()[0]
        == 0
    )
    assert [
        e for e in load_stream(db, "feature", "feat_atom") if e.type == "VERSION_ACTIVATED"
    ] == []
    assert (
        db.execute("SELECT count(*) FROM timers WHERE aggregate_id='feat_atom'").fetchone()[0] == 0
    )
...
def test_deactivate_expired_version_removes_active_entry(db):
    ...
    assert (
        db.execute(
            "SELECT count(*) FROM feature_active_versions WHERE feature_id='feat_g'"
        ).fetchone()[0]
        == 0
    )
...
def test_activation_into_blocked_use_case_is_rejected(db):
    ...
    v1 = _mint_full(
        db, "feat_blk", "run1", stamp="USEFULNESS-CHECKED", blocked=("credit_decisioning",)
    )
    res = apply_activation(
        db,
        feature_id="feat_blk",
    ...
    assert (
        db.execute(
            "SELECT count(*) FROM feature_active_versions WHERE feature_id='feat_blk'"
        ).fetchone()[0]
        == 0
    )
    ...
    assert [
        e for e in load_stream(db, "feature", "feat_blk") if e.type == "VERSION_ACTIVATED"
    ] == []
...
def test_promote_non_usefulness_checked_to_production_is_rejected(db):
    ...
    assert (
        db.execute(
            "SELECT count(*) FROM feature_active_versions WHERE feature_id='feat_stamp'"
        ).fetchone()[0]
        == 0
    )
...
def test_activation_of_version_from_another_feature_is_rejected(db):
    ...
    with pytest.raises(ValueError, match="feature_id"):
        apply_activation(
            db,
            feature_id="feat_victim",
            feature_version_id=other,
            use_case="fraud",
            base_feature_version_id=None,
            approval_type="PRODUCTION",
            actor=make_actor(),
    ...
    assert (
        db.execute(
            "SELECT count(*) FROM feature_active_versions WHERE feature_id='feat_victim'"
        ).fetchone()[0]
        == 0
    )
    assert [
        e for e in load_stream(db, "feature", "feat_victim") if e.type == "VERSION_ACTIVATED"
    ] == []
...
def test_activation_with_base_from_another_feature_is_rejected(db):
    ...
    with pytest.raises(ValueError, match="feature_id"):
        apply_activation(
            ...
            feature_version_id=v2,
            use_case="fraud",
            base_feature_version_id=foreign_base,
            approval_type="PRODUCTION",
            actor=make_actor(),
        )
    row = db.execute(
        "SELECT feature_version_id FROM feature_active_versions "
        "WHERE feature_id='feat_base' AND use_case='fraud'"
    ).fetchone()
