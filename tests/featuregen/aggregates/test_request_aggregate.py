...
def test_create_run_under_nonexistent_request_rejected(db):
    res = create_run_command(
        db,
        make_cmd(
            "create_run", "request", "req_does_not_exist", {"request_id": "req_does_not_exist"}
        ),
    )
...
def test_select_candidate_rejects_non_candidate_run(db):
    ...
    res = select_candidate_command(
        db,
        make_cmd(
            "select_candidate", "request", req, {"selections": ({"run_id": "run_not_a_candidate"},)}
        ),
    )
    assert not res.accepted
    assert res.produced_event_ids == ()
...
def test_duplicate_of_links_existing_feature(db):
    req = _open_request(db)
    duplicate_of_command(
        db, make_cmd("duplicate_of", "request", req, {"duplicate_of_feature_id": "feat_existing"})
    )
...
def test_select_candidate_mints_feature_and_closes_siblings(db):
    ...
    assert res.accepted
    assert load_stream(db, "feature", _feature_of(db, req))  # selected candidate minted a feature
...
def test_one_request_yields_multiple_features(db):
    ...
    select_candidate_command(
        db,
        make_cmd(
            "select_candidate", "request", req, {"selections": ({"run_id": a}, {"run_id": b})}
        ),
    )
    features = {
        e.payload["feature_id"]
        for e in load_stream(db, "request", req)
        if e.type == "CANDIDATE_SELECTED"
    }
