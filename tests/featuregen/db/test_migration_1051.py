...
def test_1051_applies_with_its_real_neighbours_and_none_of_the_unwritten_reservations() -> None:
    """D7 reservation reality at this branch: 1043/1045/1047 exist, 1044/1046/1048-1050 do not.
    1051 must therefore stand on the tree as it IS — a dependency on an unwritten number would be a
    migration that cannot apply."""
    names = {p.name for p in _MIGRATION_DIR.glob("*.sql")}
    present = {n for n in names if n.startswith(("1043_", "1045_", "1047_", "1051_"))}
    assert len(present) == 4, sorted(present)
    for absent in ("1044_", "1046_", "1048_", "1049_", "1050_"):
