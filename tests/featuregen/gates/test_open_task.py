...
def test_cancel_task_marks_status_and_cancels_timers(db):
    ...
    assert (
        db.execute("SELECT status FROM human_tasks WHERE task_id=%s", (task_id,)).fetchone()[0]
        == "cancelled"
    )
...
def test_cancel_tasks_on_run_advance_cancels_all_open_for_run(db):
    ...
    t2 = open_task(
        db,
        _spec(gate="CLARIFICATION", required_inputs=("b_ref",), allowed_responses=("answer",)),
        _svc(),
    )
    assert t1 != t2
    ...
    assert n == 2
    statuses = {
        s[0] for s in db.execute("SELECT status FROM human_tasks WHERE run_id='run_1'").fetchall()
    }
    assert statuses == {"cancelled"}
    sched = db.execute("SELECT count(*) FROM timers WHERE status='scheduled'").fetchone()[0]
    assert sched == 0
