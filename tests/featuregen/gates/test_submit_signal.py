...
def test_duplicate_subject_is_idempotent(db):
    ...
    submit_human_signal(
        db, task_id, response="confirm", actor=_owner("user:do1"), expected_task_version=1
    )
    again = submit_human_signal(
        db, task_id, response="confirm", actor=_owner("user:do1"), expected_task_version=1
    )
...
def test_distinct_quorum_of_two_completes(db):
    task_id = _open(db, quorum_required=2)
    r1 = submit_human_signal(
        db, task_id, response="confirm", actor=_owner("user:do1"), expected_task_version=1
    )
    assert r1.quorum_met is False
    r2 = submit_human_signal(
        db, task_id, response="confirm", actor=_owner("user:do2"), expected_task_version=1
    )
...
def test_conflicting_quorum_escalates(db):
    ...
    res = submit_human_signal(
        db, task_id, response="reject", actor=_owner("user:do2"), expected_task_version=1
    )
...
def test_ineligible_role_rejected(db):
    ...
    with pytest.raises(IneligibleResponderError):
        submit_human_signal(db, task_id, response="confirm", actor=wrong, expected_task_version=1)
...
def test_wrong_scope_rejected(db):
    ...
    with pytest.raises(IneligibleResponderError):
        submit_human_signal(
            db, task_id, response="confirm", actor=wrong_scope, expected_task_version=1
        )
...
def test_response_not_allowed_rejected(db):
    ...
    with pytest.raises(ResponseNotAllowedError):
        submit_human_signal(
            db, task_id, response="maybe", actor=_owner("user:do1"), expected_task_version=1
        )
...
def test_delegation_requires_grant_and_validates_principal(db):
    ...
    with pytest.raises(IneligibleResponderError):
        grant_task_delegation(
            db,
            task_id,
            principal=ineligible,
            delegate_subject="user:assistant",
    ...
    grant_task_delegation(
        db, task_id, principal=principal, delegate_subject="user:assistant", granted_by=principal
    )
    res = submit_human_signal(
        db,
        task_id,
