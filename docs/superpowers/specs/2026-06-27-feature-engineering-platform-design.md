...
- **Banking-vertical scope — closed boundary, open use-case set (§15.5).** This is a banking platform, not a general-purpose feature store — but *within* banking it is **open: any banking feature may be built.** The only hard gate is the **banking boundary** (banking entities, data classes, glossary, regulatory frameworks — first-class, not configuration); a request **outside banking is rejected** at intake. A **new banking use-case not yet catalogued is *onboarded*, not rejected** (§15.6). (Orthogonal to *vendor*-neutrality: vendor-neutral on technology, deliberately domain-specific on banking.)
...
Feature engineering is domain-specific: the *same* raw data yields different features, on different permitted columns, against different targets, under different governance, depending on the **business domain (use-case)**. The **Domain / Use-Case Catalog** is the Layer 0 foundation artifact that encodes this — a sibling of the Metadata Overlay (§6). Its **scope is banking** (a closed boundary) but its **use-case set is open and grows** (§15.5). Like the overlay it is **integrated/curated, not invented per feature**, is versioned, and is confirmed by the responsible owners (the domain/risk owner for domain facts; **Compliance** for policy facts, §6.5).
...
> Every feature must fall **within the banking boundary** (§15.5) — that is the only hard gate; a **non-banking** request is rejected. Within banking, **any feature may be built.** A feature's `use_case` resolves to a Domain Catalog entry for the fast path (templates + ready governance); a **new banking use-case is *onboarded*, not rejected** (§15.6) — its facts are defined and confirmed, the entry added, and it proceeds. Generation, grounding, scoring, and governance key off the entry once it exists.
...
"Banking-only" constrains the **boundary**, not the set of features. *Within* banking the platform is **open: any banking feature may be built.**

**Closed — the banking boundary (the only hard gate):**
- **Banking entities** are first-class; the entity/grain resolver (Layer 3) resolves only customer, account, product, transaction, counterparty, exposure, application.
- **Banking data classes** and a **banking business glossary** (delinquency, DPD, charge-off, utilization, SAR, structuring, churn/attrition, …) prime intake/generation so concepts map to *banking* meaning.
- **Banking regulatory frameworks** are built-in, not configured: model risk (SR 11-7-style), fair lending / disparate impact, adverse-action explainability (ECOA-style), AML/BSA, PII/data-residency — they parameterize the governance guards (Layer 7) and the policy-aware mapper (§12).

**Open — the use-case set (grows over time):** the catalog *seeds* known banking use-cases (e.g. `retail_churn`, `credit_origination`, `aml_transaction_monitoring`, …) with templates + ready governance, but it is **not a fixed allow-list**. A banking feature whose use-case is new is **onboarded (§15.6), not refused.**

**What is rejected:** only requests **outside banking** (no banking entity/data/glossary concept — e.g. "recommend Netflix shows", "e-commerce cart abandonment"); plus per-use-case **policy** violations (e.g. protected attributes for lending) and **out-of-scope latency** (real-time card fraud, §1.4). The platform never silently builds a *non-banking* feature — and never blocks a legitimate *new banking* one.

### 15.6 Onboarding a new banking use-case

When intake meets an in-boundary banking request whose use-case isn't catalogued, it **onboards** rather than rejects, inside the normal human-gated flow:

1. Confirm it is in-boundary (banking entity + data + glossary concepts).
2. At the clarification gate (Layer 2), define the new use-case's facts — target, allowed/blocked data classes, feature templates, **risk tier**, regulatory posture.
3. The **domain/risk owner confirms domain facts; Compliance confirms policy facts** (§6.5).
4. The entry is added to the catalog (versioned); the feature proceeds.

So **building is open across all of banking**; **production promotion still requires the use-case's governance to be defined and confirmed** — banking-grade control without blocking innovation.
