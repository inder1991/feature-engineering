...
10. **Positive gold coverage is mandatory** (finding #7) — each gate includes immutable positive cases that MUST resolve with the exact expected physical landing, paths, slots, and expression. A reject-everything implementation fails the gate. Resolution *rate on real traffic* stays descriptive.

## Dependency: concept authority provisioning (finding #5)

B accepts only `(SOURCE,ATTESTED)`/`(HUMAN,CONFIRMED)` concept evidence, but **no production writer attests `concept` today**: enrichment writes `LLM/PROPOSED`; the new source-capability profiles attest `unit/currency/entity/data_type/...` but **not** `concept`; no human-confirmed concept writer exists. On real traffic B would classify every operand `CONCEPT_AUTHORITY_MISSING`. Before 3C.2b-ii, an explicit authority-provisioning step is required (extend the source-capability profile to attest `concept` for governed sources, or add a human concept-confirmation writer), and B's gate must prove a **non-vacuous** attested/confirmed population — not just "zero false resolves."
