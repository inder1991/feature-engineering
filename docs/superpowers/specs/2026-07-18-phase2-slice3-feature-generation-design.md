...
4. **Confirmation `[F3]`:** `confirm_contract` (`govern.py:42`) hardcodes `"DESIGN-CHECKED"` at three write sites. Write `draft.validation_status` there instead. A feature confirmed while `NEEDS_EXTERNAL_VALIDATION` is persisted honestly, not silently promoted to `DESIGN-CHECKED`.
5. **Persistence (the immutable versioned artifact):** add `contract.validation_status text` (+ CHECK) and `contract.requirements jsonb` to the `contract` table (re-confirm = a new row, so history is preserved). Optionally mirror the status onto `feature.verification`.
