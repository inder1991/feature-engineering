...
| **C-A3c** | **[R19] `MeasureFactsReaderV1`** — unit and currency through **`read_verified_decision_value`** (no governed-field restriction), logical type and grain through the existing readers. Pin producer · strength · decision id · policy version · evidence hash into bound-input/occurrence provenance. **Without this the conversion tooth cannot fire at all — and today it degrades SILENTLY, since `not_operational` is not a hard-fail status, so a monetary operand is treated as non-monetary with nothing recorded. The reader must therefore REFUSE an unreadable measure fact rather than yield `""` [R19]** | a source-attested `per_row` currency on `txn_amt` is READ, and the pilot proposal without a conversion ref refuses `CURRENCY_CONVERSION_UNDECLARED` **through the authoring path**, not a hand-built literal; **an unreadable unit/currency refuses by name instead of silently reading as non-monetary** | S4: the provenance is pinned on the occurrence |
