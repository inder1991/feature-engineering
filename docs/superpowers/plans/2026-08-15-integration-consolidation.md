...
| **C-A3c** | **BUILT AND WIRED (live path) [R19.4]** — `formula/measure_facts.py` + `authoring_v2._read_c1_facts_v2`. **Measured on a real seeded column** (`c1_fixtures.seed_not_operational`, source-ATTESTED `unit`): `read_operational_value` → `status='not_operational'`, **`value='dollars'`** — *the value was present and `_fact_text` discarded it*; `read_verified_decision_value` → `status='resolved'`, `value='dollars'`; `read_measure_facts` → `unit='dollars'`, `resolved`, `producer=SOURCE`. Three-way disposition: `resolved` → the fact · `no_decision`/`no_value` → ABSENT · everything else incl. `retired` and any unknown status → **refuse**. An unreadable fact appends an `AuthorityFailure`, which the existing code already short-circuits to `needs_authority` without consulting the resolver. **The FROZEN snapshot path is NOT wired** — it projects without a connection and its builder hardcodes `not_operational`, so fixing it moves `snapshot_item_hash`; **D3's refusal is therefore not yet live on the recipe-driven path** | **full suite 11586 passed / 20 skipped**; `test_measure_facts.py` **14 passed** incl. a DB-backed test that a live source-attested unit is **never** answered ABSENT; `ruff`+`mypy` clean | S4: provenance pinned on the occurrence · **frozen-snapshot half still owed** |
