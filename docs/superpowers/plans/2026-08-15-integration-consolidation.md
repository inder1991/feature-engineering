...
**DONE** — `materialize/output_resolution_v2.py` + `materialize/bound_formula_store.py` + migration
`1076` (file only), `test_bound_formula_store_s5.py` **38 passed**. This is the
`OUTPUT_POLICY_RESOLVED` stage C-A7 deliberately withheld: a V3 authoring run is terminal at
`OUTPUT_INTENT_CAPTURED` because *"a stage that has not run must not be represented as having run and
agreed"*, and S5 is the stage running.

* **Clause 1 is tested as a DISCRIMINATOR, not a caution.** The same unit disagreement PASSES when
  the author stated nothing and REFUSES once they state it — the only thing that moves is
  `authored_expectation_present`. A test that checked only the silent case would pass against a
  resolver comparing nothing at all. `ADVISORY_FIELDS` (`unit`/`additivity`/`target_currency`, from
  the model's `expected_output`, absent for a deterministic run) are compared only where recorded;
  `STRUCTURAL_FIELDS` (`conversion_required`/`declared_conversion_ref`/`numeric_shape`, read from
  the formula's own shape) always are. The two sets are asserted **disjoint and exhaustive over the
  intent's comparable fields** — a field in neither would be a rule with a hole.
* **`additivity` is the sharpest case and it is not hypothetical.** `derive_output_intent_v2` never
  ...
  stopped being vacuous**. It sits in `ADVISORY_FIELDS` rather than being special-cased, and a test
  drives the REAL deriver to prove the skip against what it actually produces.
* **`derived_from_proposal_hash` is finally checked.** The type says it exists so *"an intent that
  travelled away from its formula can be caught instead of trusted"* — and nothing checked it until
  here. Checked FIRST, before any comparison, because every later disagreement would otherwise be
  attributed to an author who never saw this formula.
* **Clause 2 is enforced by the SCHEMA, not just the dataclass.** `bound_formula_revision` has no
  `compiler_version` column — asserted against `information_schema`, so it is "there is nowhere to
  write it", not "we do not write it". The version lives on `bound_formula_compilation`, one row per
  compile: recompiling under a new toolchain finds the revision present and appends a second
  compilation, so the identity holds AND the fact that a second compiler produced it is not lost —
  the same split 1074 made for inventory observations. The other side is tested too (different bound
  inputs, or a different output policy, DO mint a new revision), so the stability is a rule rather
  than an inability to tell things apart.
* **Declared is not executable, in the database as well as the type.** `currency_code` is CHECKed
  `^[A-Z]{3}$`, so `'converted:<ref>'` cannot be lifted into the field a consumer reads as "the
  currency"; the conversion ref has its own column. A monetary column with no currency code is
  refused by constraint.
* **Mismatches are stored WITH `compared_fields`.** The acceptance is a claim about the comparison
  scope, so a stored refusal that did not carry it could not be checked against the rule afterwards
  — and that rule is the thing most likely to be quietly widened later. The scope is DERIVED from
  the intent inside the writer (asserted on its signature) so a stored mismatch cannot claim a scope
  the intent did not have.
* Mutation-checked: dropping the two `is not None` guards kills 7 tests; moving `compiler_version`
  into `identity_payload` kills 4, two of them C-C6a's own.
...
**DONE** — `materialize/gate2_v2.py` + `contract_v2.py` + `group_plan_v2.py` + migration `1077`
(file only), `test_chain_v2_s6.py` **34 passed**. Driven off a REALLY COMPILED V1 IR against the
seeded catalog, the way `test_boundary_v2` is — a hand-built fixture would prove only that the V2
types accept objects shaped the way the test shapes them.

* **Not a second Gate 2.** The verdict is EXTRACTED from `authorize_compilation` as
  `ir.decide_read_scope` and V1 now delegates to it, so "V1 bytes unchanged" is a consequence of the
  structure rather than a promise. Two copies of the refusal text would both pass today and drift on
  the first edit to either — a test asserts V1's function body no longer contains the message
  (checked against comment-and-docstring-stripped source, since the docstring still explains what it
  delegates) and that V1 and a direct `decide_read_scope` call produce **byte-identical details**.
* **Clause 3 goes through the ORDINARY path**, not a rule written for FX. Policy reads fold into the
  same union as everything else via a new `ReadElementKind.POLICY_READ`, so an unreadable rate column
  refuses `READ_SCOPE_INSUFFICIENT` with the kind in the message — because "change the formula" and
  "be granted a rate table the formula never mentions" are different remedies. Discriminated three
  ways: the same group without the FX read authorizes, a readable FX column authorizes, and an
  UNGOVERNED one refuses `COLUMN_NOT_GOVERNED` first (existence before scope, inherited not
  re-decided). V1 passes no policy reads and cannot — the default is a fact about V1.
* **Clause 1 is asserted by OBJECT IDENTITY, not equality.** Two equal planned IRs carrying different
  read sets is exactly the failure the clause exists to stop, and an equality check would pass it.
  `token.planned[0] is planned`, and the downstream signatures are checked to take the TOKEN.
* **Clause 2's discriminator is the point.** The swapped policy column is NOT in
  `structural_read_set` and IS in `read_set` — asserted directly — so a formula-only leakage check
  would have passed it. The finding's `source` is `policy_read:currency_conversion`, which is what
  sends an author to the policy rather than to the formula.
* **The V2 contract classifies the PLANNED union**, proved by construction rather than by reading:
  flooring the FX rate column `restricted` moves the converted feature's `sensitivity_class` and
  leaves the unconverted one's alone, and the FX column is in the union only because it is a policy
  read. `classify_read_set`, the monotonic override (`tighten_classification`, renamed from
  `_tighten` and now shared) and `override_availability_promise` are V1's, imported not restated.
* **Clause 4 is both directions, environment-scoped per F3.** `members_of_group` and
  `group_of_feature`, keyed on `(environment_id, logical_group_name)` — the same logical group in
  another environment returns nothing. There is deliberately **no `feature_name` column**: a
  feature's published column name IS its name in a plan, so a second column holding the same string
  could only ever agree or be wrong. `ir_hash` is read from the plan's own `PlannedFeature`
  (asserted on the writer's signature: no `ir_hashes` parameter), so membership cannot claim a
  compiled plan the recorded plan disagrees with.
* **Clause 5's second half**: a group of ONE is where a stray normalization would hide — every member
  agrees with itself, so only the bytes can tell. `group_by_contract_v2` is asserted to return the
  same contract OBJECT and the same hash. Two disagreeing contracts are LISTED, never merged.
* `key_columns_from_refs` was generalized out of `group_plan._key_columns` and V1 delegates: two
  spellings of "the published row is keyed by columns" is how a V2 group ends up keyed differently
  from the V1 group beside it.
* Mutation-checked: dropping `policy_reads` from the V2 union kills 5 tests (including via
  `AuthorizedCompilationV2`'s own "the group reads X which the token does not authorize" invariant);
  rebuilding the planned IRs in the gate kills the identity test.
...
**DONE** — `materialize/seal_v2.py` + migration `1078` (file only), `test_seal_v2_s7.py`
**25 passed**.

* **Clause 1's load-bearing half is "INTACT".** That deleting the gate refuses is C-C10's and was
  already proved on the graph; what S7 adds is that the refusal does not take the evidence with it.
  `seal_v2` records the artifact, the verdict **and** the realization links on BOTH paths and
  refuses only to mark it servable — otherwise an auditor asking "which policies were applied here"
  gets an answer for every artifact that succeeded and silence for every one that did not. Proved
  for the missing-gate case AND for the harder positional one (a gate that exists, is genuinely
  consumed, and sits on a branch that never sees the joined rows).
* **An untriggered requirement is not stored as a pass.** `triggered_requirements` rides beside the
  boolean, because a boolean would report "FX subgraph sound" for a fixed-base-currency feature that
  has no FX join — claiming an inspection nobody ran. The migration additionally CHECKs that a pass
  carries no findings and a refusal carries some, so a stored row whose two halves disagree is
  unconstructible rather than resolved by whichever field a reader trusts.
* **Clause 2 is two different questions, not one.** Retrieval asks whether the store returned what
  it was asked for; execution asks whether the bytes are still what was retrieved. A pipeline
  verifying once would run whatever arrived in between, so `serve_artifact(at="retrieval")` and
  `verify_for_execution(at="execution")` are separate entry points and both are tested — plus a
  missing blob, a foreign manifest, and the write-time check that leaves **nothing** behind.
* **Clause 3 needed the LOADERS, and that is the honest finding.** "Survives a restart" is not
  testable with a second connection — the suite runs each test in one transaction, so a second
  connection sees an empty database and the test would prove the fixture. What a restart destroys is
  in-process state, so `load_sealed_artifact` and `load_manifest` were written and the test
  reconstructs everything from the artifact id alone. The verdict is REBUILT from stored fields, never
  re-derived: the graph is not persisted, and re-deriving would answer with today's requirement set
  rather than the one the artifact was sealed under. A refused artifact loads and stays refused.
* **Migration 1078 does not re-declare 1086's tables.** The plan reserved 1078 for "artifact file
  manifest + the content store" but C-D4 already shipped both as 1086, and `apply_migrations` ledgers
  by filename stem AND byte checksum — two filenames for one pair of tables is exactly F5's hazard.
  1078 holds what is genuinely left: the sealed artifact, its verdict, and the realization links.
* A structural test asserts no `lru_cache`/`@cache` in either module, because a cache added later
  would make the restart test pass for the wrong reason.
