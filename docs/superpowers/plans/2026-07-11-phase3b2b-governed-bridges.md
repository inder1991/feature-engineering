...
| Active state is a projection, not a mutable flag (#10) | Task 4 `project_verified_bridge`/`active_bridges` fold the event stream DIRECTLY (`bridge_projection.py`); `entity_bridge_edge` is derived. NOTE: a bridge is deliberately SKIPPED by the generic `OverlayProjection` (a bridge is two-source; the single-`catalog_source` read models don't model it) — so the bridge's active state comes from the direct fold, not `overlay_fact_state`. |
...
| Bound to catalog identity; drift → re-review (#10) | PARTIAL. The propose→confirm→verified→project→reject/demote cycle is delivered on the overlay_fact spine. But **expire/stale/drift-staling for a bridge is DEFERRED to 3B.3**: the generic `OverlayProjection` skips `entity_bridge` events, so a bridge is NOT registered in `overlay_fact_dependency` and cannot be drift-staled yet. 3B.3 (the first consumer) integrates the bridge's two endpoints into the projection/dependency layer to deliver drift → re-review. |
...
- **Bridge integration into the generic `OverlayProjection` (drift/expire/stale) → 3B.3.** 3B.2B SKIPS `entity_bridge` events in `OverlayProjection.apply` (a bridge is two-source; `_catalog_source`/the single-`catalog_source` read models don't model it — an unskipped bridge event would degrade+halt the whole overlay projection). 3B.3 adds an `entity_bridge` branch to `dependencies.py::fact_dependencies` (both endpoints → `overlay_fact_dependency`, mirroring approved_join) and a representative-source branch to `projection.py::_catalog_source`, so a bridge participates in expiry/stale/drift-staling. Until then, a bridge's active state is the direct fold (`project_verified_bridge`), and drift is not detected.
