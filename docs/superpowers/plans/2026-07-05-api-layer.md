...
- **LLM client wiring (post-M2/M4):** ONE optional app-level `LLMClient` powers both ingest
  enrichment and feature-assist. `create_app(llm_client=…)` injects it (tests);
  `create_app_from_env()` builds the real `ClaudeLLM` adapter iff `FEATUREGEN_LLM_PROVIDER=anthropic`
  (`featuregen.intake.llm_claude.build_claude_llm`). With no client: ingest runs un-enriched
  (concept/domain/definition absent — a supported mode) and assist endpoints return 503. NEVER
  silently swap in FakeLLM (Decision D5).
...
- Produces: `create_app(llm_client: LLMClient | None = None) -> FastAPI`;
  `create_app_from_env() -> FastAPI` (uvicorn factory entrypoint, env-gated ClaudeLLM);
  `get_conn() -> Iterator[psycopg.Connection]`; `get_identity(...) -> IdentityEnvelope`;
  `get_llm(request) -> LLMClient` (503 when absent); `get_llm_optional(request) -> LLMClient | None`;
...
**Interfaces:**
- Consumes: `get_conn`, `get_identity`, `get_llm_optional` (Task 1); `read_csv_rows`,
  `read_excel_rows`, `ingest_upload`, `IngestResult` (backend).
- Produces: `POST /uploads` (multipart `file` + form `source`) → 200 JSON
  ...
  401 without `X-User`. Enrichment runs iff the app has an LLM client (env-gated / injected).
...
def create_upload(
    ...
    client: LLMClient | None = Depends(get_llm_optional),
) -> IngestResult:
    try:
...
Run: `uv run pytest tests/featuregen/api/test_uploads.py -q`
Expected: 9 passed. (If `asserted != 4`, inspect `_table_facts` expectations rather than loosening
