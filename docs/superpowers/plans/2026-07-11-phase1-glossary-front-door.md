# Phase 1 — Unified File Ingestion (Glossary Front Door) Implementation Plan (v3)
...
**Goal:** Stand up the **unified file-ingestion pattern** (spec §U) — every file upload flows through one evidence→authority→resolution→graph flow as a *source profile*, reusing the existing technical-CSV pipeline as the shared spine. Prove it end-to-end on the FTR **glossary** profile (searchable, evidence+authority-carrying catalog), with the **technical CSV** converging via its own profile through the same entry. **No join or grain promotion.**

**v3 changes (unified-ingestion reframe — spec §U):** this is no longer "a glossary reader *additive beside* the technical path." It is the **one file-ingestion pattern**; the technical CSV is a source profile within it (not a frozen branch). Concretely: a `TECHNICAL_CSV_PROFILE` alongside `FTR_GLOSSARY_PROFILE` (Task 3); **one profile-aware validation** subsuming `validate_rows` (Task 4) instead of a glossary-special validator; the existing `validate_rows`/`_assert_fact`/`build_graph`/quarantine are **reuse points**, not parallel code; the declared-fact→`propose_fact` reconciliation (spec §U.3) is a documented, staged convergence (the working technical path stays unchanged in Phase 1). All v2 corrections (below) are retained.
...
## Global Constraints
- **Unified file-ingestion pattern (spec §U).** There is ONE file-ingestion flow for CSV/Excel/glossary; the technical CSV is a source profile within it, not a parallel branch. **Reuse the existing upload spine** — `read_csv_rows`/`read_excel_rows`/`_headers.py`, `validate_rows` (made profile-aware), `_assert_fact`/`append_overlay_event`/`resolve_fact`, `build_graph`, quarantine — do NOT build a parallel glossary pipeline. New code is only the evidence/producers/policies/resolution layer. The technical-CSV path's existing behaviour (declared facts → `build_graph`) stays UNCHANGED in Phase 1; routing declared facts as source-attested evidence (§U.3) is a staged follow-on, explicitly out of Phase-1 scope.
...
- [ ] **Step 1: Failing test** — `FTR_GLOSSARY_PROFILE`: `strength_for(profile,"definition")==ATTESTED`, `strength_for(profile,"domain")==PROPOSED`, `strength_for(profile,"sensitivity")==PROPOSED`; an unknown field defaults to PROPOSED. `TECHNICAL_CSV_PROFILE`: `strength_for(profile,"type")==ATTESTED` and `attests("type")` is True (the technical profile vouches structure) while `FTR_GLOSSARY_PROFILE.attests("type")` is False (glossary does not attest physical type — drives Task-4's profile-aware validation).
- [ ] **Step 3:** `SourceCapabilityProfile(source_type, attested_fields, proposed_fields, structural_fields)` + `attests(field) -> bool` (field in attested_fields ∪ structural_fields); define BOTH profiles (spec §U.1):
  - `FTR_GLOSSARY_PROFILE = SourceCapabilityProfile("ftr_glossary", attested_fields={"definition","business_term","bian_path","fibo_path"}, proposed_fields={"domain","sample_profile","sensitivity"}, structural_fields=frozenset())`
  - `TECHNICAL_CSV_PROFILE = SourceCapabilityProfile("technical_csv", attested_fields={"definition","sensitivity","additivity","unit","currency","entity"}, proposed_fields=frozenset(), structural_fields={"type","grain","joins_to","cardinality"})`
  `strength_for` returns ATTESTED for attested/structural fields else PROPOSED. `profile_for_upload(headers)` picks the profile (glossary-shaped → FTR; else technical). Evidence-writing (Tasks 6/10) + validation (Task 4) use these — never a literal.
- [ ] **Step 4/5:** Green; commit `feat(upload): source capability profile for FTR glossary (spec §3.3)`.

---

## Task 4: Glossary reader + **profile-aware** validation (reuse `validate_rows`; `type="unknown"`, schema preserved)
**Files:** Create `overlay/upload/glossary_reader.py`; Modify `overlay/upload/canonical.py` (or a thin `profile_validate.py` wrapping `validate_rows`); Modify `api/routes/uploads.py`; Test `tests/featuregen/overlay/upload/test_glossary_reader.py`, `test_profile_validate.py`.
**Fixes (review #1, #2, #3 + §U):** ONE profile-aware validation subsuming the existing `validate_rows` (NOT a glossary-special validator); rows emit `type="unknown"` (not `""`), schema preserved, validation accepts unknown only when the profile does not attest `type`.
...
- [ ] **Step 3b:** `validate_upload(rows, *, profile, catalog_source)` — a **profile-aware** validation that REUSES the existing `validate_rows` logic (read it first): keep all its checks (source/table/column present, source-mismatch, sensitivity, dedup/conflict) EXCEPT the hard `type` requirement, which becomes conditional: `type` is required only when `profile.attests("type")` (technical → required, as today; glossary → `type="unknown"` passes). Do NOT fork a second validator — either add a `profile` param to `validate_rows` (preferred, keeps one validator) or a thin wrapper that calls it with type-checking gated. Quarantine reasons are unchanged for technical uploads; for glossary, only invalid-FQN / missing-identity / source-mismatch quarantine (missing physical type is a Task-9 readiness blocker, per the failure-class table). Add a test proving a TECHNICAL upload with a missing `type` still quarantines (today's behaviour preserved) while a GLOSSARY upload with `type="unknown"` passes.
