...
Contracts/schema come FIRST so downstream tasks stay green (F16). Adds in `contracts.py`: reason codes `aggregation_ordering_column_missing`, `compile_disabled`; `ReplayFreshness.incompatible`; the store enums (`PlannerOutcome`/`CompileStatus`{`complete`,`incomplete`,`not_applicable`,`compile_disabled`}/`IncompleteReason`{`budget_count`,`budget_time`,`error`}/`CaptureStatus`); enrich `CatalogStateStampV1` (+`compiler_input_fingerprint`, `projection_checkpoint`, both defaulting so existing constructors still compile); real `ROLE_RESOLUTION_VERSION`.
...
**F10 signing (external trust root):** reuse the `security/audit.py` model — a `sign(digest, key) / verify(digest, sig, key)` over an EXTERNAL key from config (`get_settings()`, a `FEATUREGEN_INTENT_GATE_SIGNING_KEY` or ed25519 keypair whose PUBLIC key/keyring is a config input, NEVER embedded in the artifact). Specify algorithm, canonical bytes, key id, signer policy, a detached signature SIDECAR, a `verify_report(path, trusted_key)` that recomputes + checks, nonzero exit on any failure.
