...
> ▲ **AMENDED 2026-08-24 (owner ruling, recorded by Task P1 of the serving plan) — the gate is SPLIT into implementation vs activation.**
> **IMPLEMENTATION begins now.** The Stage-2 serving program is implemented by `2026-08-24-cross-catalog-serving-first-card.md` (Rev 13), which starts immediately by owner ruling — ahead of criteria (4)–(5). Stage-1 rigor applies; nothing merges/pushes/deploys/flips without explicit user go.
> **ACTIVATION — targeted-cohort included — stays gated** on ALL of: (a) signed SME thresholds (1C-4, with its named signatory); (b) accepted wave-1 evidence (the 1C-2 report meeting those thresholds, SME-reviewed); (c) accepted planner latency benchmarks (worker-side p95 + query ceilings); (d) `FEATUREGEN_MATERIALIZE_INVENTORY` configured plus inventory health (`materialize/queue_lane.py:213` — distinct from `FEATUREGEN_MATERIALIZE_ENABLED`; fail-closed when unset); (e) the signed gate artifact valid + enforced. **T0-verified caveat on (e):** the signed-gate prong (`overlay/upload/contract/live_activation.py:195-224`) is INERT when no `FEATUREGEN_INTENT_GATE_PUBLIC_KEY` is configured (`:211-214` returns True by deployment posture) — configuring the trusted public key is PART OF meeting this criterion, not optional; an unconfigured key does not satisfy (e).
> **R8 cross-reference (recorded in both docs):** the serving plan's supersession of `2026-08-23-parent-child-production-readiness-remediation.md` is NARROWED — its B0a/B0b own that plan's preserved Tasks 4/5/8 work (server-derived scope, target lineage, frozen method identity); see the 2026-08-24 supersession record at the top of that document.
...
- **S2-P4 — Exact plan identity via `PlanEnvelopeV2`.** A versioned sealed plan contract carrying per-segment direction, cardinality, relationship id+revision, the temporal-declaration hash (now catalog-qualified per S1A-4), and complete dependency evidence — the segment data exists (V16/§V's `BindingPathSegmentV1`); V1's flattened strings drop it. Canonicalization, version registry, persistence and migration are in scope; compilation and generation admission compare the FULL envelope, and the fan-out fixture (two same-read-set join shapes; one must refuse) is the acceptance test. Read-set equality alone never protects banking aggregates from double counting.
  ▲ **AMENDED 2026-08-24 (P1) — the sealed plan contract IS the serving plan's lifecycle identity chain, superseding this item's single-envelope `PlanEnvelopeV2` phrasing.** Implementing document: `2026-08-24-cross-catalog-serving-first-card.md` (its §Identity chain, extending `CompilationIdentityV2` + `generate_v2`'s rendered-file hashing): `logical_digest → formula_binding_digest → member_execution_input_digest → member_compile_digest → build_compilation_digest → project_digest → sealed_artifact_identity`. One versioned envelope object is NOT built; the obligations stand under the chain — per-segment direction/cardinality/relationship id+revision and catalog-qualified temporal semantics ride the logical/physical plan layers whose digests enter the chain; complete dependency evidence rides the pinned realization/dependency snapshot (the child's A4 dependency-snapshot store, T0 mapping row 1131, plus R11's dependency rows and the `pinned_dependency_snapshot_id` argument of `assess_realization_for_preview`) referenced by the physical layer and revalidated at preview assessment; compilation and generation admission compare the FULL chain (never read-set equality), and the fan-out fixture (two same-read-set join shapes; one must refuse) remains the acceptance test. Persistence per the serving plan's T0-assigned mapping (its 1134/1138 rows).
...
- **S2-P6 — Execution-source compatibility gate.** Before generation: governed physical source/connection binding per participating catalog; verify engine/connector compatibility, credentials/service identity, read authorization, data-residency policy, PIT snapshot compatibility, availability/freshness, permitted data movement, currency/reference-data dependencies. Same-substrate reachability alone proves nothing; federated/staged execution is out of scope and refused by name.
  ▲ **AMENDED 2026-08-24 (P1) — SPLIT into two compatibilities.** (1) **Render compatibility** — catalog-qualified dataset rendering, wrong-catalog-spine refusal, the `RENDERER_VERSION` bump — is IN the serving plan's scope (`2026-08-24-cross-catalog-serving-first-card.md`, its C1/compiler-renderer item): GENERATE_PREVIEW needs only this. (2) **Execution-source compatibility** — this item's eight-point checklist (engine/connector compatibility; credentials/service identity; read authorization; data-residency policy; PIT snapshot compatibility; availability/freshness; permitted data movement; currency/reference-data dependencies) over governed physical source/connection bindings — blocks **EXECUTE_SANDBOX** with `EXECUTION_SOURCE_COMPATIBILITY_UNPROVEN` (a build-item code — zero `src/` hits today; registered in the closed reason registry when built) and **NEVER blocks GENERATE_PREVIEW**. The full eight-point checklist stays chartered here, unreduced.
