...
| intake | **ENGINE + STORAGE BUILT 2026-08-10** (increments 1–2): `extract_intake_ticket` — selection-never-generation, pin-wins-with-contradiction, all-four-input cache key, degrade-never-block; storage decision executed (draft in `structured_result`, human confirmation on `contract_intent`, migration 1059 + D7 row); `/contract/intake` + `/contract/intake/target` — author-only signing, read-scope-validated refs, off-vocabulary domains REFUSED (a human decision is recorded verbatim or not at all), `user_typed` pins recorded click-free and never clobbering a signed reading; the leakage gate's existing `intent_target_ref` read consumes the signed value with zero extra wiring. Build note: one-common-word column names (`balance`) over-pin on ordinary prose — accepted for now because a pin shows-doesn't-gate with a one-click edit; revisit if pin-correction telemetry says otherwise. Increment 3 SHIPPED the confirm screen (three buttons, contradiction warning, ranked runner-up one-click corrections — prompt v2 `runner_up_refs`, selection-validated — plus the free-text field; the exploring answer renders its honest-asymmetry banner on LLM-origin cards). Post-review fixes 2026-08-10: a NEW pin may overwrite a PRIOR pin (never a human-signed reading); the signed-reading lookup is MODE-BLIND (the intake route mints hypothesis-mode intents while a definition-carrying round runs in definition mode — same question, same person, same signature). Target threading is CLIENT-side (the signed ref rides the considered-set request); the SERVER-side preference (absent `target_ref` ⟹ read the signed reading) is deliberately unbuilt — raw API callers must pass their target — and is the one remaining item |
| 3 | **BUILT 2026-08-10** (flag `FEATUREGEN_NEAR_LABEL_CRITIC`, default OFF): every surviving candidate — anchor included, ORIGIN-BLIND per the owner decision — carries `no_finding`/`too_close`/`abstain`; zero removals by construction (annotation only; the refusal mode remains a later product decision). The label window is the intake build's SIGNED reading, looked up by (hypothesis, actor) identity — MODE-BLIND after the 2026-08-10 review fix — so the legacy fresh-intent path and definition-mode rounds alike find the signature; no signed window ⟹ all-abstain at zero model cost. Verdicts content-addressed (`near_label_verdict` in `structured_result`), ledger-capped 24/build, `no_finding` deliberately not a clearance; `too_close` renders the ⚠ chip + rationale on the Workbench card. REMAINING: quality evaluation on real disagreements before any refusal-mode discussion |
