...
**Acceptance: DONE 2026-08-11 (`c1fe51ca`).** All 21 legacy recipes (12 DEPOSITS + 9 MARKETS) explicitly replaced by 25 atomic V2 definitions plus TWO risk-model ModelFeatureSpecs. THE ACCEPTANCE SENTENCES AS STRUCTURE: no deposit recipe claims the liability itself is HQLA — `hqla_eligibility_contribution` is RETIRED into `liability_cash_outflow_contribution` (the liability's actual LCR contribution) and a separate `asset_hqla_buffer` on the ASSET side, with "a liability is never a high-quality liquid asset" written into the replacement's own exclusions and pinned by test; deposit beta CANNOT compute without the customer rate — the paid rate and the benchmark are two DISTINCT operands (binding group), the paid rate carries `economic_role="paid_deposit_rate"`, and benchmark-only sources are excluded by name; LCR/NSFR contributions use regulatory classification factors — runoff class, insured status, ASF factor and residual maturity are policy-input operands under effective-dated `risk_corridor:` classifications; market model outputs preserve complete model/valuation provenance — VaR and Greeks are governed_model_output recipes over registered `risk_model_output` specs (unversioned → honestly MODEL_SPEC_BLOCKED), their eligibility refusing rows missing model version, horizon, confidence, currency or scenario. Further structure: the maturity ladder and contractual profile are CONTRACTUAL-FUTURE anchored with the horizon policy declared ("contract terms knowable AT the cutoff" — the audited temporal gap on maturity_ladder_runoff/contractual_deposit_maturity_profile, closed) and a deposit appears in exactly ONE bucket per as-of; the repricing gap runs at book/bucket grain with customer-grain aggregation excluded by name; early withdrawal reads the closure EVENT against origination, contractual maturity and the notice_period concept (BR-10's, closing that admission); net interest flow carries its SIGN AUTHORITY (income/expense as distinct economic-role legs under `direction_sign:` — the audit's degrade note closed); netting nets ONLY inside legally enforceable sets with effective credit support, unenforceable reads GROSS, post-cutoff collateral excluded; counterparty recipes run at legal-entity (lei) grain; the limit family splits usage and breach against the governed limit record; desk concentration reads the declared hierarchy. Migration counter 76→55, baseline regenerated same commit. DEVIATIONS (deliberate): operational-deposit status and counterparty class ride the LCR classification policy's declaration schema rather than separate operands (one classification authority, not four); expectations/gold ride the standing BR-7 deferral.
...
**Acceptance: DONE 2026-08-11 (`e0f69c26`).** All 43 legacy recipes (8 custody + 8 asset-management + 10 insurance + 8 Islamic + 9 ESG) explicitly replaced by 48 atomic V2 definitions plus TWO more ModelFeatureSpecs (mortality_morbidity_loading, claims_fraud_score — the plan's model reclassifications, unversioned → MODEL_SPEC_BLOCKED honestly). THE ACCEPTANCE SENTENCES AS STRUCTURE: every specialist output identifies its accounting/valuation/methodology basis — a governed policy ref or a status/policy-input operand, total over the packs by test; claims count, severity inputs and loss ratio are SEPARATE outputs with CORRECT additivity (count and paid additive; loss ratio non-additive over EARNED premium under the earning basis — written and collected premium are their own recipes); Islamic recipes carry contract-specific semantics — contract structure and Sharia-board governance as policy inputs, profit-rate words with conventional-lending terminology structurally absent (pinned by test over every recipe's prose), Murabaha reading due_date/scheduled_amount/payment_allocation with the principal/profit split policy (the old admission closed); ESG outputs cannot combine incompatible boundaries, years or methodologies — the BOUNDARY is an operand (policy input under `risk_corridor:ghg-boundary-methodology-year`), cross-boundary summation excluded in the aggregation contract itself, absolute emissions and intensity separate recipes, PCAF attribution + data quality carried, physical risk scenario-named. Custody: trade/contractual/actual settlement dates DISTINCT (binding group), a fail knowable only AFTER contractual settlement (stated on every fail recipe), the fail family split count/value/rate/age, matching read from matching_status (the stage before settlement — that admission closed). Asset management: investor flows separate from market performance, versioned NAVs under the valuation calendar (superseded strikes excluded), governed benchmark identity, redemption terms/gates policy. Insurance admissions closed by BR-10 concepts: policy_loan_balance vs surrender_value, customer_income for adequacy, product_holding for bancassurance; lapse censoring names competing risks. Migration counter 55→12, baseline regenerated same commit. DEVIATIONS (deliberate): the five per-family test files are ONE module (same assertions, one harness); the redemption-risk model spec arrives when a model registers (historical net flow is the deterministic recipe today); Takaful claims and participant-fund state ride the claim/fund concepts rather than new ones; expectations/gold ride the standing BR-7 deferral.
...
**Acceptance: DONE 2026-08-11 (`11d3bc8a`) — AND THE MIGRATION COUNTER REACHES ZERO.** All 12 legacy corporate recipes explicitly replaced by 15 atomic V2 definitions, and with this final family pack every one of the 157 legacy recipes has an explicit V2 replacement: `legacy_recipes_not_in_v2` is 0, PINNED BY TEST over the whole registry. THE ACCEPTANCE SENTENCES AS STRUCTURE: no LC/guarantee output computes from generic contingent exposure alone — the instrument's identity, type and lifecycle event kind (BR-10's `lc_guarantee_event`) are operands, and a true rollover is distinguished from an amendment-and-extension by the event vocabulary with the confusion excluded by name; DSO, dilution and debtor concentration require invoice lifecycle data — invoice identity, status, issue/due/paid dates and debtor identity are operands, payment flows with no invoice linkage excluded (an invoice partially paid then credited counts its credited portion in DILUTION, not DSO); group exposure respects the effective legal hierarchy and elimination policy — membership effective AT the as-of (a departing subsidiary leaves the aggregate at that date), intra-group facilities eliminated, at legal-group grain; the intraday peak CANNOT compile from daily snapshots — its source grain IS the intraday sweep event with minute-window semantics, the EOD utilization is a separate recipe on the daily grain, and "an EOD snapshot is not evidence of intraday behaviour" is the empty-population answer. Further: SCF reads buyer/supplier/program with the program limit as governed threshold (a supplier changing programs re-binds, never blends); covenant breaches carry waiver/cure state (a waived breach is never a pass); guarantees count only enforceable+unexpired under the wrong-way policy; the working-capital cycle reads declared AR/AP/revenue periods (the BR-6 signed-sum exemplar's shape); `obligor_facility_count` stays atomic and honestly FORMULA_AUTHORABLE on its reviewed Formula-v1 expectation; stressed-line count is its own atom against the governed stress threshold with "a limit is not exposure" held. Baseline regenerated same commit. DEVIATIONS (deliberate): RAROC/relationship-profitability outputs were never legacy recipes and arrive with their component packs; gold scenarios ride the standing BR-7 deferral (encoded meanwhile as eligibility structure).
...
**Acceptance: DONE 2026-08-11 (`3b153516`).** THE CUTOVER, with every acceptance sentence structural: contract v3 uses ONLY RecipeDefinitionV2 — every one of the 157 legacy template ids resolves through the SOURCE-CONTROLLED alias map (derived from `replaces_legacy_ids`, never heuristic) to its atomic replacements, carries `recipe-contract-v2`, presents its BEST-ready atom through BR-7's real fold (an idea with an authorable realization says so, even while a conceptual sibling stays an idea) and NAMES every replacement in the additive `v2_replacements` field (route model + frontend type + drawer rendering updated); a multi-measure legacy id REFUSES to resolve without its output alias (`AmbiguousLegacyAlias` names the atoms — 27 split ids, the one-output rule held at the seam); UNASSESSED survives only as the unknown-template fallback a real page never contains (pinned over ALL_TEMPLATES). All reviewed debt counters equal ZERO for the active registry — `legacy_recipes_not_in_v2` 0, variant identity collisions 0, UNASSESSED unconstructible, every V2 recipe declares its primary, output ids unique — pinned by the strict-mode test. Existing v1/v2 API snapshots unchanged (their byte-stability tests untouched and green; generation stays on the legacy projection for the compatibility window). Historical canonical-recipe-v1 contexts still verify (the v1 hash pin untouched). The legacy registry is FROZEN — its id set pinned exactly (new Template authoring fails CI, which is also BR-2's routing rule enforced for the 4c triage cards) — and marked read-only with removal criteria in its own docstring (deleted when v1/v2 retire at BR-24 and no stored revision resolves a legacy id). Release coverage reads the ACTIVE registry: zero legacy applicability inference (structurally — every definition declares its objectives), the census now 51 AUTHORED_PRIMARY / 24 ZERO / 13 INTENTIONALLY_EMPTY, the coverage-targets backlog REGENERATED onto the owning R5 expansion packs (BR-18 foundation / BR-19 customer-wealth / BR-21 ecosystem), the Phase-0 exit pin superseded by "covered or scheduled". Stale docs updated (registry-empty and 153-recipe wording gone). DEVIATIONS (deliberate): gate1/legacy-adapter untouched — v1/v2 generation is the compatibility window's own surface and moves only at BR-24; audit strict mode is the pinned zero-counter TEST (CI-enforced) rather than a CLI flag flip, because the legacy registry's historical counters legitimately stay nonzero in the same report.
...
**Acceptance: DONE 2026-08-11 (three commits: `fe3f3e66` tranche A + expectation seam, `f1a1b18b` account, tranche B following) — with the end-to-end caveat recorded honestly below.** SIXTY-THREE new atomic primitives (24 transaction tranche A + 19 tranche B analyticals + 20 account), all at account grain, none replacing anything (the contract pin now distinguishes migration-era packs, which must replace, from foundation packs, which must not). THE SEAM THE PACKS WAITED FOR: `recipe_formula_expectations_v2` — a v2 expectation IS a reviewed gold_v2 fixture pinned by canonical hash (the v1 manifest's no-self-refresh freeze for the v2 generation; fixture-side proof in tests), and `has_reviewed_expectation` is ONE gate for both generations, wired into the v3 fold and release coverage. THE CANONICAL EXEMPLAR: `posted_debit_amount` demonstrates the complete contract the plan spells out — account grain over the transaction event grain, the full operand set (transaction_id, account, amount, direction, status, booking + value stage timestamps, correction link), ALL FOUR governed policies, trailing window, additive over account and time within one currency, base-currency conversion only through the governed rate, empty window zero, null policy reviewed — and is the pack's ONE honestly-FORMULA_AUTHORABLE recipe, its reviewed expectation being the BR-6 gold exemplar; every other primitive is FORMULA_BLOCKED until its own review, pinned. Acceptance sentences structural: reversals and failed transactions CANNOT inflate posted activity (posted recipes exclude them by name and policy; the correction family counts them in its own population with the original-transaction link); mixed currencies CANNOT sum without the governed conversion (every additive monetary output carries the policy, pinned); account and customer grains are NOT conflated (every output grain is account, pinned — rollups are separate recipes per the plan, the collections pack's exemplar being the pattern). Tranche B waited for its operations correctly: all 19 use operations the BR-6 grammar already accepted; percentile and payment category are identity-bearing SEMANTIC parameters (nine distinct percentile variant identities proven through the BR-3 machinery); the FX spread is rate-authority-gated ("no authority, no spread"). Six coverage backlog rows retired by real primaries (engagement, cross_border, deposit_stability, liquidity, limit_management, overdraft_propensity — gaps 24→18). DEVIATIONS (deliberate): "gold-validated and compiled end to end" is the BR-22/24 evaluation machinery's half of the acceptance — the exemplar's gold fixture pins its canonical form today, and runtime gold-data cases + compilation ride the engine; the shadow/eval gate run likewise; account-to-customer rollup exemplars beyond the established pattern, source-eligibility examples for the four feed shapes, and wide-table perf tests ride BR-22's harness; overlaps with existing atoms are absent by design (one id, one recipe — salary regularity, round-amount share, turnover, net flow, recency named in the module docstrings).
...
**Acceptance: DONE 2026-08-11.** TWENTY-NINE recipes across the three packs plus TWO more registered ModelFeatureSpecs (churn_probability, campaign_uplift — the plan's model-governance references for churn and uplift, joining the existing propensity and CLV specs; 8 registered model features total). THE ACCEPTANCE SENTENCES AS STRUCTURE: a customer feature can explain exactly which accounts and allocation rules contributed — the contributing account is an OPERAND and the allocation rule a POLICY INPUT on relationship_balance and relationship_revenue, with "every contributing account listable" in the aggregation contract itself; no next-best-action suggestion bypasses eligibility or suitability — the NBA machinery is the cross-sell pack's (eligible-universe operands, suitability policy on the propensity recipe), and this pack adds the deterministic/prediction SEPARATION the rules demand (uplift requires treatment AND control — "never uplift without a control group" is the spec's own fallback policy; the descriptive response rate stays its deterministic sibling); wealth performance carries its bases — the portfolio facts carry the valuation/FX policy and base currency (performance itself lives in the asset-management pack with benchmark identity and fee basis, one recipe one home); previously supporting-only objectives gain reviewed primaries where data permits — primacy_loss (salary_anchoring_ceased_flag reading the GOVERNED designation, operating_balance_share_trend), customer.clv (relationship_revenue — the REALIZED value the projection trains on), wealth.asset_outflow (contribution and outflow SPLIT — never netted — plus cash drag), leaving the backlog at 15 with every remaining gap owned by BR-21. Privacy/suitability gates structural: contactability, vulnerability and risk-profile mismatch each carry their privacy_purpose policy as an operand ("no purpose, no number"; vulnerability "never derived from behaviour"); household counts require VERIFIED effective-dated membership; the intentionally-empty client-attrition leaf receives NOTHING (pinned). DEVIATIONS (deliberate): customer_relationship_tenure and channel diversity exist already as atoms (tenure_days, channel_adoption_depth — one id, one recipe); account-level RBWM product primitives largely exist in the foundation/credit packs (card utilization = utilization_level, overdraft usage = the account pack's overdraft family, DD continuity = the retail mandate recipes) — this pack adds the behaviours that did NOT exist; wealth realized/unrealized performance rides performance_vs_benchmark; gold cases ride the standing BR-22 deferral.
...
**Acceptance: DONE 2026-08-11.** TWENTY new recipes across the four packs, extending beyond the corrected corporate atoms (which already carry much of the plan's menu — the module docstrings name every deliberate non-duplication). THE ACCEPTANCE SENTENCES AS STRUCTURE: CIB features distinguish the grains — `CIB_GRAINS` is the explicit vocabulary (legal party, obligor, legal group, account, facility, instrument, pool, client) and every BR-20 output grain is a member, pinned; trade-finance outputs follow the INSTRUMENT LIFECYCLE — the LC/guarantee stage is an identity-bearing SEMANTIC parameter (issuance/amendment/utilization/expiry/claim: five features by identity, one body), generic exposure excluded by name, and documentary/SCF durations subtract two NAMED stage timestamps in distinct binding groups; profitability metrics STATE their methodologies or refuse — direct service cost carries its allocation policy as an operand, while RAROC, revenue wallet share and hedge effectiveness stay CONCEPTUAL with the missing governed methodology named (the plan's own blocked list, held verbatim); intraday liquidity CANNOT compile from end-of-day data — the intraday recipe's source grain IS the intraday position event with minute windows, EOD stand-ins excluded by name (the pool-level twin already held this in the corporate pack). Two more backlog leaves get reviewed primaries: aml_cft.kyc (periodic-review overdue against the refresh schedule) and treasury_alm.cash_management (cash-position volatility, intraday peak usage, sweep behaviour, virtual-account utilization) — backlog 15→13, every remaining gap fraud/aml/credit-underwriting owned by BR-21. Further structure: the facility maturity wall and refinancing concentration are CONTRACTUAL-FUTURE anchored ("exactly one bucket per as-of"); FX hedge ratio counts only DESIGNATED hedges under the linkage policy; treasury-product adoption fences sales outcomes from predictors by leakage declaration; contingent-to-funded conversion reads both exposure roles at two named reads. DEVIATIONS (deliberate): the packs author what did NOT exist — operating balances, product revenue, penetration, limit/excess, group exposure, covenant headroom, DSO/dilution, SCF utilization, pool utilization/intraday, working-capital cycle and cross-product stress all live in their earlier packs (one id, one recipe); client/account/facility profitability ROLLUPS follow the established rollup pattern on demand; gold cases ride the standing BR-22 deferral.
...
**Acceptance: DONE 2026-08-11 — AND THE COVERAGE PROGRAM COMPLETES.** THIRTEEN recipes across the two accepted packs (lending_lifecycle: 5, fincrime_expansion: 8), and the census the whole tier machinery was built to reach: ZERO gaps — all 75 coverable selectable leaves carry a reviewed V2 primary, the 13 intentionally-empty leaves keep their owner-role and rationale rows, the targets backlog is EMPTY, and the state is PINNED so a future regression reopens loudly. THE ADMISSION RULE'S STRUCTURAL HALF, PINNED: no pack was added merely to turn a leaf green — every recipe answers a NAMED end-user decision in its decision_context (an underwriting affordability call, a vintage read, an SME lending call, a mitigation reliance question, a recoveries return question, and the eight typology/control questions), with one atomic output, exact source and output grains, a formula path, declared PIT/knowledge semantics and policy constraints. New recipes enter DIRECTLY as V2 — the legacy adapter is structurally unavailable to them (the registry freeze makes new Templates fail CI). The honest shapes: affordability reads customer_income against committed outgoings under the assessment basis ("never defaulted"); mitigation counts only ENFORCEABLE mitigants after haircut ("never face-value comfort"); recovery cash is leakage-class OUTCOME; the account-takeover signal is a SEQUENCE of two named event reads ("the sequence IS the signal" — never a composite score); synthetic identity reads the thin-file flag through bureau KNOWLEDGE time; sanctions and screening get CONTROL-STATE facts (pending hits with "absence of feed is not zero hits"; coverage with unknown counted UNSCREENED); structuring gains persistence beside the smurfing count under the jurisdictional threshold; correspondent banking gets its DD review state; and mule-account/TBML stay INTENTIONALLY EMPTY exactly as the taxonomy owner ruled, pinned. Priorities B/D/E/F were found substantially COVERED by the earlier packs (the ALM, operations-adjacent, profitability and specialist recipes named in those packs' rows) — pack priority followed data readiness and user value, not taxonomy vanity, which for those priorities meant NOT re-authoring what exists. DEVIATIONS (deliberate): the admission rule's named-SME-owner and non-vacuous-gold-case halves are the REVIEW track's deliverables — owners are recorded as ROLES until the BR-23/BR-24 sign-off list attaches names (the honest-absence rule: never invent people), and gold cases land with BR-22's harness, where every FORMULA_BLOCKED readiness in these packs already points.
...
**Acceptance: DONE 2026-08-11 (increment 1 — the meaning-level corpus; the engine-executed half recorded below).** THE ACCEPTANCE SENTENCES AS STRUCTURE: the old semantic defects each fail at least one test — the twelve plan-listed adversarial mutations each trip a NAMED guard (auth-for-settlement swaps re-key identity and are classifier-refused; deposit-balance-as-drawn is role-gated and identity-bearing; removing the sign authority re-keys; measure-without-additivity and unmatched-PIT-parameters are UNCONSTRUCTIBLE; formula edits stale reviews by canonical-hash lookup miss; late group membership is fenced by the effective snapshot; a later model version re-keys the spec; uplift-without-control is refused by the spec's own fallback; the second-currency-without-conversion mutation exercises the BR-6 refusal directly); a formula implementation CANNOT pass by reusing its own computation — the reference evaluator is test-side and self-contained (zero production imports) and the expected values are HAND-COMPUTED literals with the arithmetic written in the gold file for independent re-derivation, with the mid-window FX rate change making any single-rate shortcut produce 73.00 or 74.00 where only at-booking conversion yields 73.50 (pinned as its own test); every ACTIVE executable recipe has linked gold — the two v1 anchors through the 28-case v1 corpus, the exemplar through this one, held as a registry-wide assertion so a fourth authorable recipe without gold fails CI; no fixture contains production data — fabricated ACC* synthetics, PII-marker scan pinned. The corpus: a reviewable JSON ledger covering the posted/pending/failed/reversed lifecycle with boundary rows NAMED in the gold file and their presence pinned; EIGHT property invariants as ledger mutations (ineligible rows inert, exact reversals neutralize, post-cutoff moves subtract exactly, unrelated currencies inert, additive flows partition over disjoint windows, account splits preserve allocated totals, ratios survive equal scaling, ineligible duplicates inert) with the remaining additivity invariants held at the contract layer; the LEAKAGE BOUNDARY over the WHOLE registry (every outcome recipe prohibits pre-default modelling, every near-label prohibits origination, no permit/prohibit overlap, alert feeds read through knowledge time, and standard recipes carry NO fences — a fence on everything is a fence on nothing); the corpus version and content hash published in `gold_corpus_manifest.json` under the no-self-refresh discipline. Test layers 1-5 and 8-11 live in their existing suites (schema/lint/grounding/authority/formula = the BR-1..6 batteries; API/UI = the BR-8/17 route and card suites) — this task added the layers only IT could add: row semantics, PIT-by-mutation, property invariants, registry mutation, leakage-as-population. DEVIATIONS (deliberate, the engine-gated half): differential tests between authored formula and COMPILED artifact, per-recipe gold for every future executable, the shadow/eval gate run and the performance budgets require the execution engine (BR-24-era) — the corpus, the manifest identity and the linkage law are ready for it; synthetic datasets beyond the transaction ledger arrive with each family's first expectation review, using this ledger's format.
...
**Acceptance: DONE 2026-08-11 (schema half in R1 per the sequencing amendment; validity/API half here).** THE ACCEPTANCE SENTENCES AS STRUCTURE: no production-active recipe lacks current revision-specific approval — `review_validity` is the pure fold BR-24's activation gate reads, and `review_coverage_report` gives it the per-family × readiness numbers; a changed formula automatically stales the previous approval — "current" is a lookup at the canonical-recipe-v2 hash (the R1 design), the fold consumes ONLY revision-specific reviews, and the POST route adds OPTIMISTIC CONCURRENCY on top (approving a definition that changed under you is a 409 naming both hashes); review history is immutable and attributable — the 1060 append-only guards make UPDATE/DELETE database errors, ordering is the 1061 append sequence, and reviewer identity comes from the SESSION subject, never the request body (pinned); conceptual recipes are SME-approvable AS IDEAS — the fold requires no engineering signature for a conceptual pattern and structurally cannot touch readiness. The role law, from each recipe's OWN declarations: SME + semantic owner always; formula_engineering for executables; model_risk for governed model outputs AND near-label/outcome leakage; privacy_compliance wherever a `privacy_purpose:` policy rides; treasury_regulatory_accounting wherever a `risk_corridor:`/`model_output:` classification rides — each derived off real registry recipes by test. Authors cannot self-approve: a multi-role requirement satisfied by one identity is a named violation. Audit links land on every event (expectation hash — the pinned v2 fixture hash for the exemplar — gold refs, policy dependencies, permitted/prohibited stages). DEVIATIONS (deliberate): review STATUS is deliberately NOT duplicated into contract v3's execution block — the review route is the ONE authority, reachable from v3's `v2_replacements` ids (duplicating DB state into the suggestions page would mint a second authority for governance state; the drawer fetches it); RecipeReviewV1 (the definition-side dataclass) needed no extension — the event store carries the full plan-listed contents; the governance SCREEN is the plan's own "optionally, after the backend event model lands"; populating review owners for all migrated recipes is the BR-24 activation checklist's human half.
...
**Acceptance: DONE 2026-08-11 (the code side — the operational half is the operator's, scripted in the runbook).** THE ACCEPTANCE SENTENCES AS STRUCTURE: V2/v3 can be disabled without data loss or historical corruption — the rollout module is CONFIG and pure folds holding no state (flipping a flag changes what serving paths consult, never what the registry contains), the v3 route's rollback lever is a typed 422 with v1/v2 byte-untouched, and the frozen legacy registry + alias map guarantee historical identities never rewrite; every promoted family passes ITS OWN gates — promotion requires the flag AND the per-family allowlist AND the per-catalog canary list, and `canary_gate` is a fold over the eight plan-named readings whose inputs DEFAULT TO THE FAILING SIDE (an unmeasured gate blocks; a family promotes only on an empty failure list, never a score); production metrics describe readiness truthfully and never use suggestion count as success — `rollout_metrics` reports readiness counts, primary and executable coverage (honestly zero executable-covered until the gold gates run) and the gold-linked authorable set, with suggestion count DELIBERATELY ABSENT and pinned so; legacy retirement is a later explicitly approved project (stage 10 in the runbook, its own decision). The four feature controls carry STAGE-HONEST frozen defaults pinned by test: v3-by-explicit-query stays ON (today's behavior, stage 3), V2 serving / Formula-v2 authoring / materialization OFF, allowlists empty — changing a default is a reviewed rollout decision, never a drive-by. `rollout_stage` derives the honestly-reached stage from configuration alone. The operator runbook (`docs/architecture/banking-recipe-rollout-runbook.md`) scripts the human half: migrations 1060+1061 backend-first with a verification read, the six levers, stage-by-stage promotion (internal v3 viewing → review population → one-family/one-catalog canary → foundation authoring → approved-recipe materialization → family-by-family expansion → v3 default), the eight gates and the tested rollback shape. DEVIATIONS (the operational half, deliberately human): the deploys themselves, dual-run comparison RECORDS, dashboards, the representative-catalog runs, SME acceptance sessions, real-evidence formula evaluation and the non-production rollback rehearsal are operator actions the runbook sequences — the platform refuses to pretend they happened (every gate input defaults to blocking until a real measurement supplies it).
