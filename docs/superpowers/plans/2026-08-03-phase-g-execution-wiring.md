...
2. Compile-side artifact retention — migration **1054**: the group plan and contract body as JSONB keyed by `generation_id`. Without it a crash after render loses the packing list §9 gates against, and no human can audit *what* a run intended to read.
   **Amended 2026-08-03 (Task 5, structural finding — not a judgement call):** prepared parameters are NOT part of this table and never can be. Two independent proofs: (a) `prepare_run` is keyed on `run_id`/`business_dt` (`runprep.py:831`), so parameters are *run*-scoped while this table is `generation_id PRIMARY KEY` — one row could hold only one run's set; (b) 1034's append-only guard function has no escape branch, so a nullable column reserved for G-2 could never be filled by an UPDATE. G-2 needs its own run-keyed table. **1055 stays reserved for G-3's active-revision pointer; 1057+ is the free range (1056 = Track-1's PII allow-policy, D14) but claiming it requires appending to the Track-1-owned D7 reservation table in the same commit — a coordination step, not a unilateral one.**
...
**A.40** (renumbered from A.33 at the tree unification — Track-1 holds A.33) (the orphaned 1020 authoring lane), **A.41** (renumbered from A.34 — Track-1 holds A.34) (L0 probes inside the compile transaction),
