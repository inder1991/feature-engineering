...
2. Compile-side artifact retention — migration **1054**: the group plan, contract body and prepared parameters as JSONB keyed by `generation_id`. Without it a crash after render loses the packing list §9 gates against, and no human can audit *what* a run intended to read.
