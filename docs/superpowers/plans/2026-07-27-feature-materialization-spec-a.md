...
- [x] **Step 1: Failing tests** — a matching schema with a wrong `ir_hash` still fails (`IR_HASH_MISMATCH`) · a manifest from a **different generation/run/business_dt** ⇒ `STALE_STAGING_MANIFEST` · duplicate manifest ⇒ `DUPLICATE_STAGING_MANIFEST` · missing/failed manifest · missing/extra/mistyped column · **nullability mismatch** ⇒ `WRONG_NULLABILITY` · `expected_schema` includes the three **system columns** (`__generation_id`, `__generated_project_hash`, `__sandbox_execution_hash`) · adding a feature changes `group_plan_hash` and **keeps** the binding · a **different contract hash for the same logical name** ⇒ `PublicationRefusalCode.GROUP_BINDING_CONFLICT` · **the binding record has no mutable field**: adding a feature appends a `GroupPlanRevision`, and `current_plan_revision` DERIVES the current plan from the latest successfully-published revision · name collision is a plan error.
- [x] **Step 2–5:** Run/implement/run/commit — `feat(materialize): group plan, staging manifests, contract-bound target`

**Established (interfaces reference §28), beyond the sketch above:** the §9 gate is `check_completeness`, which returns EVERY `GateFailure` rather than the first — a gate that stopped early sends an operator round the regenerate/rerun loop once per broken feature — and it judges STALENESS **before** the `ir_hash` and **over the whole staging area**, because a manifest bound to another generation/run/date is not evidence about this run and grading its computation would report a verdict nobody asked for · `WRONG_NULLABILITY` is made expressible rather than raised here: nullability rides on `PlannedColumn` and on `group_plan_hash`, and the comparison against a real DataFrame is T14's · **the landing KEY and `business_dt` carry `sql_type=None`** — §6 types a FEATURE column and nothing governed states `cif_id`'s physical type at compile time, so declaring one would be an invention the §9 gate then enforced against a real table (A.12); their NULLABILITY *is* decided, since a NULL landing key is not a landing key · a feature can never collide with a system column **structurally**, because `hive_identifier` requires a leading letter and every system column starts `__` · the manifest's `status` is a `StrEnum` that still round-trips the rendered pipeline's JSON string, and a `failed` one is `INCOMPLETE_COMPUTATION` (the record exists and says the column was never computed) while a manifest for an unplanned feature is `UNEXPECTED_COLUMN` — §14 names neither, both recorded in A.12 · `current_plan_revision` takes the **published generation ids** as evidence, because publication success cannot live on an append-only revision row, and it refuses an ambiguous tie (two published revisions at one instant, or two plan hashes under one generation) rather than picking · `created_at` must be OFFSET-AWARE ISO 8601 and is ordered by INSTANT, since `2026-07-27T23:00:00+05:30` sorts after `…T19:00:00+00:00` as text and is 90 minutes before it in fact · `admission.hive_identifier` was made PUBLIC so the plan reaches the same answer Gate 1 already reached about which column a feature occupies. **Task 10 stayed PURE** — `bind_group` / `plan_revision` / `current_plan_revision` are functions over values, and the `group_binding` tables land with the rest of T14's control plane.
...
- [x] **Step 1: Failing tests** — `derive_namespace()` takes **no parameters**, and the module source contains no production literal · hashes are **plural** · `RenderedArtifactIdentity` is built after rendering · **`generated_project_hash` appears in `GENERATED.lock` and in no other file**, and is computed over every file *except* the lock · `sandbox_execution_hash` includes `business_dt`, `input_snapshot_ids` and the capability attestation id, and is reproducible · `CompilationIdentity` contains **none** of those run-time values · no `production_execution_hash` exists.
- [x] **Step 2–5:** Run/implement/run/commit — `feat(materialize): two-phase sandbox-only identity`

**Established (interfaces reference §29), beyond the sketch above:** *"appears in the lock and nowhere else"* is enforced by making `seal_project` the ONLY path that writes a lock — it REFUSES a renderer-supplied one, because a renderer that wrote its own would either state a hash it could not yet know or feed an older one back into the bytes being hashed. A seal-time scan for the computed hash was deliberately NOT added: a single-pass renderer cannot embed a hash it was never given, so the branch would be dead code, and T14 keeps its own *"no generated source file contains the project-hash literal"* assertion instead · `generated_project_hash` excludes the lock **whether or not it is present**, which is what lets L0 recompute it over a COMPLETE project and compare · the two hash tuples are **positionally paired** (one formula and one IR per feature), so they are neither sorted independently nor de-duplicated in `__post_init__`; `build_compilation_identity` emits them in the PLAN's order — already sorted by column name — so the order the features compiled in cannot change the identity · that builder closes a real gap: **nothing in the chain checked `PlannedFeature.ir_hash` against `ir_hash(ir)`**. Task 10's caller supplies that value and §9 gates every staging manifest against it, so a plan naming a hash no IR produced would have failed at run time against a computation nobody performed · `sandbox_execution_hash` takes the **RENDERED** identity rather than a `CompilationIdentity` plus a separate project hash, so a caller cannot pair one compilation with another project's bytes, and **nothing is defaulted** — a defaulted `capability_attestation_id` would let an execution be identified without naming the attestation §10.3 requires before anything may publish · `input_snapshot_ids` keeps the caller's ORDER (§3.4: *the exact ordered partition set read*) while `parameters`' key order is not identity (RFC 8785 sorts object keys) · `derive_namespace()` reads `binding.SANDBOX_NAMESPACE` **through the module**, so the drift test can move the constant; an `is` comparison would prove nothing, because CPython interns identifier-shaped literals and a re-spelled `"sandbox_feature"` would be the very same object · the lock is plain sorted JSON with `read_lock` as its ONE parser, since T14's rendered assembly node reads it at run time and a second parser would be a second chance to disagree about which key holds the hash. **A.13 records the compiler/renderer versions §7 names but no module owns.**
...
- [x] **Step 1: Failing tests** — every required file emitted (`settings.py`, `pipeline_registry.py`, `pipelines/materialize/{nodes,pipeline}.py`, `conf/base/*`, `GENERATED.lock`, `README.md`, `pyproject.toml`, `requirements.lock`) · pipeline wires explicit `inputs=`/`outputs=` · settings registers both hooks · README states the `kedro run` vs `spark-submit` distinction · **pinned dependency versions come from `ClusterInventoryV1.engine_versions`** · catalog names only read-set + spine tables · target is `sandbox_feature.*` and **not** parameterized · every `.py` parses · deterministic · `materialize_to` writes a real directory.
- [x] **Step 2–5:** Run/implement/run/commit — `feat(materialize): render a complete runnable Kedro project`

**Established (interfaces reference §30), beyond the sketch above:** `ClusterInventoryV1.engine_versions` **did not exist** — Task 0 is unstarted, so Task 12 added `EngineVersions` and the required field to `inventory.py` (the module Task 0 owns, extending rather than restating it) plus a **`kedro_datasets`** field the plan's sketch omits: Kedro ships its dataset implementations in a separately versioned distribution, so `spark.SparkHiveDataset` resolves out of `kedro-datasets` and a lock naming only `kedro` leaves the class that reads every governed source table unpinned · **`RENDERER_VERSION` is now owned**, in `render/__init__.py` rather than in `render/project.py`, and `sandbox_execution_hash`'s `renderer_version` PARAMETER IS GONE: `identity` is below the renderer in the import graph (`render.project` imports `identity` to seal), so the constant lives in the one module that imports nothing, `identity` reads it *through the module* exactly as `derive_namespace` reads `binding.SANDBOX_NAMESPACE`, and nothing can pass a literal because there is nowhere to pass one. `compiler_version` stays a parameter — §2's chain still has no orchestrating module to own one (A.13) · **the renderer takes Gate 2's `AuthorizedCompilation`**, not a bare list of IRs: §1.3 says no contract, group plan *or project* is produced from an unauthorized group, and the token is the only thing that shows the whole read set was authorized · **node bodies are INJECTED** (`RenderedNode`: source + imports + explicit wiring), because the catalog is the only place a storage location may be written, so the module that writes the catalog must be the module that decides the dataset names — Task 12 owns the shell and the wiring, Tasks 13/14 own every line of compute, and `render_project` refuses a node set whose wiring does not close (undeclared output, a write to a `raw` source, two writers, an unwritten dataset, an unread source, no publisher) · a **`feature_staging_assembled`** dataset was added because §9's gates run *after* assembly and *before* publication, and an undeclared output is an in-memory dataset a failed gate leaves nothing to inspect · the published entry's `write_mode` is **`errorifexists`**, fail-closed: §10 bans `INSERT OVERWRITE` and §10.3 forbids selecting any mechanism before the probe proves one, so an artifact rendered today is structurally unable to publish over anything until T16 replaces the entry · **the Kedro API surface was VERIFIED, not remembered** — kedro 1.5.0 + kedro-datasets 9.5.0 installed in a scratch venv, signatures inspected, and the rendered project's `Pipeline` object BUILT through the real `configure_project`/`register_pipelines` (10 nodes; free inputs exactly the two governed source tables; one terminal output) and its catalog loaded through the real `OmegaConfigLoader` with `${runtime_params:staging_root}` interpolating. That is more than `ast.parse` and still **not L0** — L0 installs and runs, and stays Task 15's.
...
- [x] **Step 1: Failing migration tests** — `materialization_generation`, `pipeline_validation_report`, `materialization_run_event`, `materialization_run_manifest`, `publication_capability_attestation`, `group_binding` **all** reject UPDATE, DELETE **and TRUNCATE** (statement-level `BEFORE TRUNCATE … FOR EACH STATEMENT`; a `FOR EACH ROW` trigger does not fire on TRUNCATE) · `(run_id, seq)` unique on events · closed `event_kind` CHECK · manifest FKs to `generation_id` · one terminal manifest per run.
- [x] **Step 2: Failing gate tests** — every `ValidationGateCode` rendered · assembly consumes staging manifests · **assembly adds each of the three system columns exactly once** (assert exactly one copy of each) · `__generated_project_hash` is **read from `GENERATED.lock` at runtime** and **no generated source file contains the project-hash literal** · `__sandbox_execution_hash` arrives via prepared run parameters · a failed gate raises · the manifest writer never calls `collect()`/`take()`/`head()` · `run_status` folds from events.
- [x] **Step 3–6:** Run/implement/run/commit — `feat(materialize): rendered gates, hooks, append-only control plane`

**Established (see A.22 for 14a, A.23 for 14b):** `ValidationGateCode` has **18** members, not 15, and *"every code rendered"* is false of any single project — `SPINE_NON_DETERMINISTIC` is rendered only under `LATEST_AVAILABLE_AS_OF`, so the coverage corpus is the full project plus a spine under every snapshot policy · the migration landed as **1034** (1031–1033 taken, two of them on unmerged branches) · `hooks.py` was NOT re-rendered (A.15) · `WRONG_NULLABILITY` is judged on ROWS because Spark widens the nullable flag through a left join · `SCHEMA_HASH_MISMATCH` is reported only when nothing more specific was, so its live reach is column ORDER · `PROJECT_INTEGRITY` recomputes the project hash on the cluster and skips `__pycache__` / `*.egg-info` / dot-directories · the gate node writes the publication dataset and contains no mechanism (§10.3) · A.16 answered: `RuntimeError` with the code as leading token is KEPT and made a contract by `gate_code_of`.
