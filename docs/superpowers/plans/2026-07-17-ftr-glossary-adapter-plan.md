...
## Round-5 resolutions (REAL-FILE run — user-approved; supersede Round-4 where they conflict)

Running the actual `FTR_Column_Mapping_final.csv` (127 data rows) exposed defects the synthetic fixture missed. Ground truth: 6 term_types incl. `Regulatory Term`(×1); 85 definitions carry the canonical `representative values such as …` clause; 41 say "sample profile has no non-blank values" (SAFE); all 127 definitions > 200 chars; 0 malformed-width rows; 0 unresolvable FQNs.

- **R5-1 `term_type` is OPEN (reverses Round-4 #7).** Do NOT quarantine an unknown/blank term_type. Normalize (lowercase, spaces→`_`), record the raw value (provenance), and only exact `measure` drives the Pass C join-key exclusion. Any other value — known or new — ingests. (Closed vocab quarantined a valid `Regulatory Term` column; blank bypassed it anyway.) Remove the `TERM_TYPE_VOCAB_V1` quarantine.
- **R5-2 Sanitizer: DROP the value-shape guesser (reverses Round-4 #2's token gate).** Keep only: (a) `strip_sample_values` (the canonical `representative values such as …` excision — covers 100% of the real file's samples); (b) a fail-closed marker scan for phrases that imply ACTUAL DATA survived the strip — `representative values`, `sample values`, `observed values/entries`, `example values` — but NOT bare `sample profile` (41 real definitions say "sample profile has no non-blank values" and must NOT blank); (c) `redact_free_text` for PII. Remove `_VALUE_TOKEN_RES`/`_residual_suspicion` token counting entirely (it over-blanked a real payment definition and missed `OPN; CLS; PND`). Accepted, documented tradeoff: a bare non-canonical value list with no marker is not auto-caught (the real file never does this; distinguishing it from prose is intractable). Bump `SANITIZER_VERSION` to v3.
- **R5-3 Blank-vs-missing governance hole.** A sanitizer-SUPPRESSED definition (blanked because a marker survived) must NOT be silently treated as "missing" and LLM-drafted into the graph without a governance decision. Mark suppressed definitions so `draft_definitions` skips them (leave empty + flag for review), never silently overwrite with LLM text.
- **R5-4 Schema fence keys on TABLE (fixes Round-4/#4 gap).** Within an upload, two rows under the SAME table with DIFFERENT schemas are a conflict (currently keyed by table.column, so same-table-different-columns evaded it). Cross-upload: the fence must also detect a new schema claiming an existing `public.<table>` identity even when column names don't overlap (check table-level, not only column nodes).
- **R5-5 `declared_type` reaches enrichment.** `_concept_metadata` must include `GlossaryRecord.declared_type` (the file's `VARCHAR`/`DOUBLE`/`TIMESTAMP`) as classifier input; operational `type` stays `UNKNOWN_TYPE`.
- **R5-6 Reject malformed row widths.** A row where `csv.DictReader` populates the `None` key (>17 cells, e.g. an unquoted comma shifting fields) must QUARANTINE (adapter-level, tagged), not silently ingest with shifted fields.
- **R5-7 Unresolvable-FQN rows QUARANTINE via the adapter (redo Round-4 M3).** Do NOT drop them to an identity-less row losing term/domain/taxonomy/term_type/schema/declared_type. The adapter emits a `RowError(adapter="ftr")` preserving the raw physical FQN + the record fields in the reason, so inline-repair is refused and nothing is silently lost.
- **R5-8 Honest sanitizer provenance.** Retain per-field `state`/`reason`; the parse-stage detail records honest counts distinguishing stripped vs suppressed vs PII-redacted, across ALL free-text fields (not only definition).
- **R5-9 Honest input row count.** The run/panel "rows" figure reflects the real input (127 data rows), not `len(rows)` (which drops the table term + adapter quarantines).
- **R5-10 Tests model the REAL file shape.** Fixtures use all 6 term_types (incl. `Regulatory Term`), the "sample profile has no non-blank values" phrasing, canonical `representative values such as` clauses, and >200-char definitions. The acceptance test must assert 126 columns + 1 table + **0 quarantine**, every definition PRESENT (none wrongly blanked), and the `Regulatory Term` column ingested. (Pre-existing, deferred: #10 concept-cache/evidence coupling — not A1-introduced.)
...
2. **Value-shape residual-suspicion gate (Findings #11 + T2 review Findings 1&2 — user-directed, supersedes the earlier marker-whitelist).** Do NOT delete individual values by shape (that corrupts legitimate definitions), and do NOT blank on a bare introducer phrase (that over-blanks concept prose). Instead: **(1)** strip the known FTR sample clause (`strip_sample_values`); **(2)** scan the REMAINING definition for a high-confidence value LIST — ≥2 value-shaped tokens (numeric, time `HH:MM:SS`, code e.g. `LON01`/`AB-1`, quoted `"…"`, or uppercase-entity runs e.g. `ARTKOM GLOBAL FZE`) **together with** list separators (`;`/`,`) and/or sample-context words (`values`, `entries`, `codes`, `observed`, `include`); **(3)** if a suspicious list remains → `state="suspected_unhandled"`, blank the ENTIRE definition; **(4)** allow normal concept prose (`"attributes such as tenor and rate"`, a single uppercase acronym, an ordinary lowercase taxonomy list) through unblanked; **(5)** apply PII redaction afterward as defense in depth. Bump `SANITIZER_VERSION`. Distinguish the failure reason `unhandled_marker` (a residual known sample-marker phrase like `representative values`/`sample profile`/`observed entries`) from `suspected_value_list` (value-shaped residual list) — via a defaulted `reason` field on `DefinitionSanitize` (shape otherwise unchanged, so callers are unaffected). Corpus MUST include positive AND negative cases: the demonstrated leaks (`"the values were 84848; 90210"`, `"observed entries include ARTKOM GLOBAL FZE and NORDIC HOLDINGS AS"`, `"Branch codes: LON01, NYC02, SGP03"`) → blanked; and negatives (`"attributes such as tenor and rate"`, `"a representative office in the region"`, `"sample population size"`, a single acronym like `"the GDP figure"`, an ordinary taxonomy list `"party, product, account"`) → NOT blanked; plus mixed-case entities, quoted values, dates, codes.
