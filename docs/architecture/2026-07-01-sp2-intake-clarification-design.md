...
- **Identity + command authz + structural SoD** (SP-0 §6) — `create_request`, `create_run`,
  `submit_human_signal(gate=CLARIFICATION)`, and `select_candidate` all already carry authz rows keyed to the
  **data scientist / request owner**, plus a `service:intake-agent` service principal for system-initiated
  steps (SP-0 §6.2). SP-2 introduces no new gate values.
...
4. **Handlers + a lifecycle guard set** wired into SP-0's durable runtime for the Layer-1/2 flow (§11), and
   ...
   CONFIRMED_CONTRACT transition is machine-checkable.
...
author-owned intent lock (§8). Answers are idempotent by `(task_id, subject)` and quorum-1 (SP-0 §7).
...
- The confirmer **MUST be the authenticated human requester** — **never a service principal and never the
  LLM**. Enforced by SP-0 authz on `select_candidate` / `submit_human_signal` (request-owner rows,
  SP-0 §6.2) plus an explicit **`actor_kind == human`** check. A service or LLM attempt is **denied and
  recorded in the security-audit stream** (SP-0 §6.2), never applied.
...
- **Commands:** `submit_intent(request, intent_text, intake_mode)`; `answer_clarification(task_id, actor,
  response)` (a thin wrapper over SP-0 `submit_human_signal(gate=CLARIFICATION)`); `confirm_contract` /
  `select_candidate(run_id, actor, candidate_id)` (Gate #1, over SP-0 `select_candidate`);
...
- **Clarification + Refinement Loop:** a must-ask field opens an SP-0 `CLARIFICATION` task (eligible assignee =
  request owner); answering re-normalizes, re-scores, and closes the field; the **declined-encoding** example
...
- **Human Gate #1:** author-self-confirm produces the Confirmed Contract; **a service principal or the LLM is
  denied → security-audit** (never applied); confirmer must be the authenticated requester with
  `actor_kind=human`; **selected + rejected candidates + assumptions + human edits + ambiguity notes +
