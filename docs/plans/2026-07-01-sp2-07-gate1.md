...
- Produces:
  ...
  - `_open_gate1_task(conn, run_id, *, actor) -> CommandResult` — the internal, actor-agnostic opener reused by `request_edit` (Task 7.6). Guards the MCV gate (folded status `MINIMUM_CONTRACT_VALIDATED`) + `open_fields_empty`; cancels pending clarification tasks; opens the dedicated confirm task (`gate="CLARIFICATION"`, `allowed_responses=("confirm","edit","reject")`, `delegation_allowed=False`, `eligible_assignees={"role":"data_scientist","subject":<owner>}`, `required_inputs=(<final Draft doc-id>,)`).
...
**Interfaces:**
- Consumes: `_confirmer_is_requester_human` / `_deny_audited` / `_final_draft` / `_request_id` / `_freeze_contract_doc` / `_open_gate1_task` (Tasks 7.1–7.2); `submit_human_signal(response="edit", …)` (task-version OCC); **R5** `run_minimum_contract_validation` (P5 — re-run, reads `.accepted`); **R1** `append_feature_contract_event` (P1, aliased `append_fc_event`); `UNKNOWN` (SP-0 draft sentinel).
- Produces:
