...
- **P1 (`intake/store.py`, `intake/events.py`, migrations `0508`/`0509`, authz):**
  - `append_feature_contract_event(conn, *, run_id: str, type: str, payload: Mapping, actor: IdentityEnvelope, request_id: str | None = None, provenance: ProvenanceEnvelope | None = None, expected_version: int | None = None, caused_by: str | None = None) -> EventEnvelope` — **R1**, the ONE Feature-Contract append seam in `intake/store.py` (owned by P1 Task 1.3; mirrors SP-1's `overlay/store.py::append_overlay_event`): appends on `aggregate="feature_contract"`, `aggregate_id == feature_contract_id == run_id`, sets the typed mirror-id and `request_id=request_id` for correlation, keeping `feature_id IS NULL` per the `0508` `events_aggregate_id_consistent` `feature_contract` branch. Imported here (this phase aliases it `from featuregen.intake.store import append_feature_contract_event as append_fc_event`); **never redefined** and **never INSERTs into `events` directly** (Global Constraint).
...
- **P2 (`intake/contract.py`, `intake/state.py`, `intake/banking_catalog.py`, `intake/catalog.py`):**
  - `assemble_confirmed(draft_body: dict, *, confirmation: dict, derived_from: tuple[str, ...], requires_independent_validation: bool, chosen_method, feature_name: str | None = None) -> dict` — **R7**, deterministic Draft→Confirmed **renames + `calculation_method` reshape** (`entity_grain`→`feature_grain` + derived `entity_key`; `proposed_feature_name`→`feature_name`; string method → tagged `calculation_method`) that ALSO stamps `status="CONFIRMED"`, the `confirmation` record, and `requires_independent_validation`. `confirm_contract` passes ALL of these (no re-derivation, no passthrough monkeypatch).
  - `validate_semantics(body, *, stage) -> None` — **R6**, raises `ContractSemanticError`; `confirm_contract` calls `validate_semantics(confirmed_body, stage="CONFIRMED_CONTRACT")`.
  - `classify_intent(intent: str, *, product: str | None = None, region: str | None = None, catalog) -> IntakeClassification` (attrs `.outcome: IntakeOutcome`, `.catalog_version: str` [**R9** — NOT `.version`], `.matched_class: str | None`, `.as_mapping()`); `IntakeOutcome` enum (`CLEAR`/`OUT_OF_SCOPE`/`PROHIBITED_DATA_CLASS`/`SENSITIVE_PROXY_CLARIFY`/`AMBIGUOUS_CLARIFY`/`NEEDS_USE_CASE_ONBOARDING`); **R8** the catalog seam `current_intake_catalog() -> BankingDomainCatalog` (module-global, fail-closed if unset; attr `.version`) in `intake/catalog.py` — `_prohibited_intent_screen` uses this, NOT `load_banking_catalog(conn)`.
- **P5 (`intake/mcv.py`):** `run_minimum_contract_validation(conn, run_id: str, *, actor: IdentityEnvelope) -> CommandResult` — **R5**, the DB-backed re-runnable 6-check MCV that folds the FC status, validates the **current** Draft, and on success appends `MINIMUM_CONTRACT_VALIDATED` (folds → `MINIMUM_CONTRACT_VALIDATED`); `request_edit` re-runs it and reads `.accepted`. (Its checklist is the pure `minimum_contract_validated(draft_body, ledger, classification) -> MCVResult`, `.passed`, which P7 does NOT call.)
- **`intake/state.py` (shared FC-fold API — the authoritative decision state every SP-2 phase folds; **R3** owned by P2 Task 2.5, consumed here unchanged):** `FeatureContractStatus` (closed enum: `NEEDS_CLARIFICATION`, `MINIMUM_CONTRACT_VALIDATED`, `CONFIRMED`, `OUT_OF_SCOPE`, `PROHIBITED_DATA_CLASS`, `NEEDS_USE_CASE_ONBOARDING`); `FeatureContractState` (attrs incl. `.status: FeatureContractStatus`, `.open_fields: tuple[str, ...]`, `.requester: str | None` [**R4** = the INTENT_SUBMITTED actor.subject]); `fold_feature_contract_state(stream) -> FeatureContractState`; **R4** `actor_is_request_owner(state, actor) -> bool` — the ONE owner predicate (P7's `_confirmer_is_requester_human` composes it with `actor_kind=="human"`).
- **P4 (`intake/commands.py`):** the module already exists with `_SP2_CATALOG` (a **tuple** of `(action, handler)` pairs) and the idempotent `register_sp2_commands()` (skips already-registered actions — mirrors SP-1's `register_overlay_commands`); this phase **appends** three pairs to `_SP2_CATALOG`. The `DRAFT_CONTRACT_PRODUCED`/`CONTRACT_REFINED` FC events carry **R12** `draft_doc_id` (the frozen Draft doc-id) + `assumption_ledger_ref` **and** `draft_body` (the full non-PII Draft body, replayable — mirrors SP-1's `OVERLAY_FACT_CONFIRMED.payload["value"]`).
- **SP-0 (verbatim):**
...
**Interfaces:**
- Consumes: `fold_feature_contract_state` / `FeatureContractStatus` / `FeatureContractState.open_fields` / `FeatureContractState.requester` / **R4** `actor_is_request_owner` (state.py); **R1** `append_feature_contract_event` (store.py, aliased `append_fc_event`) / `register_sp2_event_types` (events.py); `GateTaskSpec` + `open_task` + `cancel_tasks_on_run_advance` (SP-0 gates); `append_document` / `compute_content_hash` / `NewDocument` / `provenance_for` (SP-0 docs); `register_primary_selected` (SP-0); `_SP2_CATALOG` / `register_sp2_commands` (P4).
- Produces:
  - `open_gate1_task(conn, cmd) -> CommandResult` (reads `run_id` from `cmd.args["run_id"]` / `cmd.aggregate_id`).
  - `_open_gate1_task(conn, run_id, *, actor) -> CommandResult` — the internal, actor-agnostic opener reused by `request_edit` (Task 7.6). Guards the MCV gate (folded status `MINIMUM_CONTRACT_VALIDATED`) + `open_fields_empty`; cancels pending clarification tasks; opens the dedicated confirm task (`gate="CLARIFICATION"`, `allowed_responses=("confirm","edit","reject")`, `delegation_allowed=False`, `eligible_assignees={"role":"data_scientist","subject":<owner>}`, `required_inputs=(<final Draft doc-id>,)`). **X4 note:** this handler folds the FC status for its guards but appends **no** `feature_contract` event — it writes only `human_tasks` rows (`cancel_tasks_on_run_advance` + `open_task`). There is therefore no FC-stream append to CAS on, so the shared X4 rule (`expected_version`=folded head + `ConcurrencyError`→`stale`) has nothing to attach to here and is a no-op; X4 fires on the two FC appends — `confirm_contract`'s `CONTRACT_CONFIRMED` and `request_edit`'s `CONTRACT_REFINED`.
  - Module helpers reused by all Gate-#1 handlers: `_request_id(stream) -> str | None`, `_final_draft(stream) -> tuple[str | None, dict | None]`, `_confirmer_is_requester_human(state, actor) -> bool` (composes the **R4** `actor_is_request_owner(state, actor)` predicate with `actor_kind=="human"`; owner subject is read from `state.requester`, NOT a local re-derivation), `_deny_audited(conn, cmd, aggregate_id, reason) -> CommandResult`, `_freeze_contract_doc(conn, *, run_id, request_id, stage, body, branch_role, derived_from, supersedes, actor) -> str`.
...
class _Cls:
    def __init__(self, outcome, catalog_version, matched_class=None):
        self.outcome, self.catalog_version, self.matched_class = outcome, catalog_version, matched_class


def _fake_mcv(conn, run_id, *, actor):
    """Deterministic stand-in for P5's run_minimum_contract_validation — advances the fold to
...
def _gate1_seams(monkeypatch):
    ...
    R7: assemble_confirmed is NOT monkeypatched — confirm_contract calls the real P2 reshape,
    passing the full pinned arg set."""
    import featuregen.intake.commands as C

    monkeypatch.setattr(C, "current_intake_catalog", lambda: _StubCatalog("bdc-2026.06"), raising=False)
    monkeypatch.setattr(
        C, "classify_intent",
        lambda text, *, product=None, region=None, catalog: _Cls(IntakeOutcome.CLEAR, catalog.version),
        raising=False,
    )
    monkeypatch.setattr(C, "run_minimum_contract_validation", _fake_mcv, raising=False)
...
from featuregen.contracts import Command, CommandResult, ConcurrencyError, DbConn, NewDocument
from featuregen.contracts.gates import GateTaskSpec
...
from featuregen.intake.catalog import current_intake_catalog  # R8 — module-global catalog seam
from featuregen.intake.contract import (  # Draft→Confirmed reshape + semantic validation
    assemble_confirmed,  # R7
    validate_semantics,  # R6
)
from featuregen.intake.mcv import run_minimum_contract_validation  # R5 — DB-backed MCV (Task 7.6)
from featuregen.intake.state import (
    FeatureContractStatus,
    actor_is_request_owner,  # R4 — the ONE owner predicate
...
def _confirmer_is_requester_human(state, actor) -> bool:
    ...
    Composes the ONE R4 owner predicate (intake/state.py); SP-0 authz only admits the
    data_scientist ROLE (never the specific subject), so SP-2 pins it."""
...
**Interfaces:**
- Consumes: `fold_feature_contract_state` / `FeatureContractStatus` (state.py); `submit_human_signal` → `SignalResult(counted, status, …)` (task-version OCC); **R7** `assemble_confirmed(draft_body, *, confirmation, derived_from, requires_independent_validation, chosen_method, feature_name)` + **R6** `validate_semantics(confirmed_body, stage="CONFIRMED_CONTRACT")` (P2 `contract.py`); `_final_draft` / `_request_id` / `_freeze_contract_doc` (Task 7.1); **R1** `append_feature_contract_event` (P1, aliased `append_fc_event`); **X4** `ConcurrencyError` (SP-0 `featuregen.contracts`) — the `CONTRACT_CONFIRMED` append CASes on the folded head.
- Produces:
  - `confirm_contract(conn, cmd) -> CommandResult` — `cmd.args = {run_id, task_id, expected_task_version, feature_name?, human_edits?, ambiguity_notes?, candidate_doc_id?}`. Folds status; **no-regression** + `status==MINIMUM_CONTRACT_VALIDATED` guards; consumes the gate task via `submit_human_signal(response="confirm", expected_task_version=…)` (OCC); assembles the Confirmed body via **R7** `assemble_confirmed(draft_body, *, confirmation, derived_from, requires_independent_validation, chosen_method, feature_name)` (which stamps `status="CONFIRMED"`, the `confirmation` record, and `requires_independent_validation` — the handler does NOT re-derive them) then **R6** `validate_semantics(confirmed_body, stage="CONFIRMED_CONTRACT")`; freezes the `CONFIRMED_CONTRACT` document `derived_from` the final Draft; appends `CONTRACT_CONFIRMED` (carrying `confirmed_body` inline) **with `expected_version` = the folded-head `stream_version` (X4 CAS)** → folds status to **`CONFIRMED`**; a raised `ConcurrencyError` (a concurrent transition landed since the fold) → `stale` denial.
...
def confirm_contract(conn: DbConn, cmd: Command) -> CommandResult:
    ...
    confirmed_body = assemble_confirmed(
        ...
        chosen_method=draft_body.get("feature_semantics", {}).get("calculation_method"),
    ...
    # X4 — CAS on the folded head (§12, Global Constraints): a None-append would take the CURRENT
    # head, letting a stale fold + these writes silently land AFTER a concurrent transition. Pin
...
**Interfaces:**
- Consumes: `classify_intent` / `IntakeOutcome` (P2 `banking_catalog.py`) + **R8** `current_intake_catalog()` (P2 `catalog.py`); the folded Draft body + its intake-time `provenance.catalog_version` (P4).
- Produces:
...
**Interfaces:**
- Consumes: `_confirmer_is_requester_human` / `_deny_audited` / `_final_draft` / `_request_id` / `_freeze_contract_doc` / `_open_gate1_task` (Tasks 7.1–7.2); `submit_human_signal(response="edit", …)` (task-version OCC); **R5** `run_minimum_contract_validation` (P5 — re-run, reads `.accepted`); **R1** `append_feature_contract_event` (P1, aliased `append_fc_event`); **X4** `ConcurrencyError` (SP-0 `featuregen.contracts`) — the `CONTRACT_REFINED` append CASes on the folded head; `UNKNOWN` (SP-0 draft sentinel).
- Produces:
  - `request_edit(conn, cmd) -> CommandResult` — `cmd.args = {run_id, task_id, expected_task_version, field_edit}` where `field_edit = {"field": <dotted path>, "to": <value>, "from"?: <old>}`. Owner+human guarded (audited); OCC via `submit_human_signal(response="edit")`; supersedes the Draft with a **REVISED** `DRAFT_CONTRACT` version, appends `CONTRACT_REFINED` (folds → `NEEDS_CLARIFICATION`) carrying the revised body + `human_edits` **with `expected_version` = the folded-head `stream_version` (X4 CAS); a raised `ConcurrencyError` → `stale` denial**; then **re-runs MCV** and **re-opens** a fresh Gate #1 task — unless the edit re-introduced an `open_field` (`to == UNKNOWN`), in which case the run stays in the **Refinement Loop** (no re-open). An edit never confirms silently.
...
def request_edit(conn: DbConn, cmd: Command) -> CommandResult:
    ...
    # X4 — CAS on the folded head (§12, Global Constraints): pin expected_version to the head folded
    # above (a None-append would silently ride a concurrent transition); a raised ConcurrencyError is
    # a stale denial. The gate task was consumed above, but the caught error keeps the txn usable
    # (events store raises pre-INSERT / via savepoint) so the whole edit rolls back on the denial.
    try:
        append_fc_event(
