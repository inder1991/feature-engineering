...
- `featuregen.overlay.catalog`: `CatalogObject` (frozen: `object_ref, object_kind, schema, table, column, data_type, native_oid`), `CatalogAdapter` Protocol (`list_objects()`, `get_fact(...)`, `owner_of(ref)`, `fingerprint()`).
...
- `featuregen.overlay.commands`: `_OVERLAY_CATALOG` (the command catalog the `register_overlay_commands()` loop registers), `current_overlay_catalog() -> CatalogAdapter` + `set_overlay_catalog(adapter)` (the module-level bound adapter the command handlers resolve authority/structure through).
...
> **Assumptions flagged for the overview author** (see also the structured notes): the overview defines `profiler.py` as `run_profiler_scan(conn, adapter, ref, *, limits) -> list[Proposal]` but does **not** define the `Proposal` or `ProfilerLimits` dataclasses — this phase is authoritative for them (signatures below). The overview lists `_OVERLAY_CATALOG` + `register_overlay_commands()` for `commands.py` but does not name how command handlers obtain the bound `CatalogAdapter`; this phase consumes `current_overlay_catalog()`/`set_overlay_catalog(adapter)` and **creates them in `commands.py` if Phase 4 has not** (Task 6.3, Step 6). The `propose_fact` command **arg contract** (consumed verbatim in Task 6.3) is assumed to mirror the `OVERLAY_FACT_PROPOSED` payload keys from the overview: `catalog_object_ref, fact_type, use_case, proposed_value, proposal_fingerprint, evidence_ref`.
...
Add the `run_profiler` service command to `overlay/commands.py`. As a service principal it: resolves the bound `CatalogAdapter`, runs `run_profiler_scan`, and for each candidate computes `fact_key` + `proposal_fingerprint`, **dedups against REJECTED/DRAFT** in `overlay_proposal` (a rejected fingerprint is skipped even with fresh evidence), then `write_evidence(...)` and issues a `propose_fact` command. Registration is via the existing `_OVERLAY_CATALOG` loop.

**Files:**
- Modify: `src/featuregen/overlay/commands.py` (add `_run_profiler` handler; append `("run_profiler", _run_profiler)` to `_OVERLAY_CATALOG`; ensure `current_overlay_catalog`/`set_overlay_catalog` exist — Step 6)
- Create: `tests/featuregen/overlay/test_run_profiler_command.py`

**Interfaces:**
- Consumes: `run_profiler_scan`, `ProfilerLimits`, `PROFILE_VERSION` (Task 6.1/6.2); `fact_key`, `proposal_fingerprint`, `display_object_ref` (Phase 2 `identity`); `write_evidence` (Phase 2 `evidence`); `CatalogObjectRef`; `current_overlay_catalog()` (commands.py bound adapter); SP-0 `get_command("propose_fact")`, `Command`, `CommandResult`, `IdentityEnvelope`; the `overlay_proposal` table (overview `0083`).
...
from featuregen.overlay.catalog import CatalogObject
from featuregen.overlay.commands import set_overlay_catalog
...
Expected: FAIL — `ImportError: cannot import name 'set_overlay_catalog'` or `no matching authz policy`/`get_command('run_profiler')` KeyError (handler not registered).
...
def _run_profiler(conn, cmd: Command) -> CommandResult:
    ...
    adapter = current_overlay_catalog()
    ...
    for proposal in proposals:
        ...
        propose_cmd = Command(
            ...
            aggregate_id=fk,
            args={
                "catalog_object_ref": dict(cmd.args["ref"]),
...
- [ ] **Step 4: Ensure the bound-adapter accessor exists**

If Phase 4 did not already add it, define the module-level bound `CatalogAdapter` in `src/featuregen/overlay/commands.py` (the command handlers resolve structure/authority through it):

```python
_OVERLAY_ADAPTER: "CatalogAdapter | None" = None


def set_overlay_catalog(adapter) -> None:
    """Bind the CatalogAdapter the overlay command handlers resolve through."""
    global _OVERLAY_ADAPTER
    _OVERLAY_ADAPTER = adapter


def current_overlay_catalog():
    if _OVERLAY_ADAPTER is None:
        raise RuntimeError("overlay catalog adapter is not bound; call set_overlay_catalog(...)")
    return _OVERLAY_ADAPTER
```

> If Phase 4 already provides `current_overlay_catalog`/`set_overlay_catalog`, skip this step and import/use the existing ones (do not redefine them).
...
- [ ] **Step 6: Run the full Phase-6 suite to confirm no regressions**
