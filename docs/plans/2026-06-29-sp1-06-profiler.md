...
def _existing_proposal_fingerprint(conn, fk: str) -> tuple[str | None, str | None]:
    # Authoritative: read the EVENT STREAM, not the (asynchronous) overlay_proposal projection — so the
    # profiler never writes evidence for a proposal propose_fact would deny under projection lag (finding 2).
