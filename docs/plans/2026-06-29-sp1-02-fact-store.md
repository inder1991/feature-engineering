...
**Interfaces:**
- Consumes: `featuregen.events.registry.EventSchemaRegistry` (has `register_schema(type, ver, schema, owner, *, status)`, `validate(type, ver, body)`, `assert_writable(type, ver)`); `jsonschema`. **Does NOT import SP-0's `SchemaValidationError`** — value-shape failures raise the overlay-owned `FactValidationError` defined here (overview pin 9). The event-registry's own `validate`/`assert_writable` still raise SP-0 `SchemaValidationError`, which is exercised only by the registry tests below.
- Produces:
  ```python
  class FactValidationError(Exception)   # overlay-owned; raised by validate_fact_value (pin 9)
...
def test_availability_time_schema_accepts_good_and_rejects_malformed():
    facts.validate_fact_value(facts.AVAILABILITY_TIME, {"column": "posted_at", "basis": "posted_at"})
...
- Produces:
  ...
  def write_evidence(conn, *, fact_key, table_snapshot_at, row_count, sample_size,
                     profile_version, thresholds_used: Mapping, metric_values: Mapping,
                     created_by: Mapping) -> str   # created_by stored as jsonb (pin 14)
  def read_evidence(conn, evidence_id) -> Evidence
  ```
  - **`created_by` is a `Mapping`** persisted to the `overlay_evidence.created_by` jsonb column (pin 14). Callers pass `identity_to_jsonb(actor)` (a plain dict) — **never** a raw `IdentityEnvelope`, which is not JSON-serializable.
...
- **Symbols verbatim:** `fact_key`, `display_object_ref`, `proposal_fingerprint`, `validate_fact_value`, `FactValidationError`, `OVERLAY_EVENT_SCHEMAS`, `register_overlay_event_types`, `fold_overlay_state`, `OverlayState`, `append_overlay_event`, `load_fact`, `Evidence`, `write_evidence`, `read_evidence`, `OverlayProjection`, `current_fact`, `read_proposal`, `dependents_of` all match the Shared Contract.
- **Contract pins applied:** pin 9 (`FactValidationError` raised by `validate_fact_value(..., use_case=None)`), pin 11 (`proposed_by` is the actor subject STRING), pin 14 (`write_evidence(..., created_by: Mapping)` stores jsonb; callers pass `identity_to_jsonb(actor)`), pin 18 (dependency rows DELETEd + refreshed on every (re)proposal).
