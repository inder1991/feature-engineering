...
- `events` accepts `aggregate="overlay_fact"` (migration `0080_overlay_events.sql`) and `aggregates/_append.py::append` takes an `overlay_fact_id` keyword that threads to `EventEnvelope.overlay_fact_id`.
- The overlay read/evidence/dependency tables and the `'overlay'` projection-checkpoint row exist (migration `0083_overlay_tables.sql`).
...
def test_approved_join_schema_accepts_good_and_rejects_malformed():
    ...
    with pytest.raises(SchemaValidationError):
        facts.validate_fact_value(
            facts.APPROVED_JOIN,
            {"from_columns": ["customer_id"], "to_columns": ["id"], "cardinality": "many"},
            None,
        )
...
FACT_VALUE_SCHEMAS: dict[str, dict] = {
    ...
    APPROVED_JOIN: {
        "type": "object",
        "required": ["from_columns", "to_ref", "to_columns", "cardinality"],
        "properties": {
            "from_columns": {"type": "array", "items": {"type": "string"}, "minItems": 1},
            "to_ref": _CATALOG_OBJECT_REF_SCHEMA,
            "to_columns": {"type": "array", "items": {"type": "string"}, "minItems": 1},
...
def _dependencies(object_ref: str, fact_type: str, value: Mapping) -> set[str]:
    """Object refs a fact's value references (§8 general dependency index): the keyed object plus
    grain.columns / availability_time.column / scd valid_from+valid_to / both sides of an
    approved_join. A drop/rename/type-change to ANY of these stales the dependent fact."""
    deps: set[str] = {object_ref}
    ...
    elif fact_type == facts.APPROVED_JOIN:
        to = value["to_ref"]
        to_obj = ".".join(p for p in [to["schema"], to["table"]] if p)
        deps.add(to_obj)
        deps |= {f"{object_ref}.{c}" for c in value.get("from_columns", [])}
        deps |= {f"{to_obj}.{c}" for c in value.get("to_columns", [])}
    return deps
    ...
    def apply(self, conn: DbConn, event: EventEnvelope) -> None:
        ...
        if event.type == facts.OVERLAY_FACT_PROPOSED:
            ...
            conn.execute(
                ...
                "updated_seq = %s WHERE fact_key = %s AND updated_seq < %s",
