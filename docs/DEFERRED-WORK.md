...
| ⚪ **The reader-visible POINTER SWITCH is not rendered — CLOSED 2026-08-15 (D3), as a SEAM rather than as rendered DDL** | Still not rendered, and deliberately: the switch is `publish.PublicationSwap`, a Protocol the DEPLOYMENT implements, called by `publish_generation` after §9's gates. Rendering the DDL would have put the operation inside the artifact, where the probe's attestation says nothing about it. **No implementation exists in `src/`** — the package invariant is "render-only, never imports pyspark" — so `swap=None` on the lane config means this deployment cannot publish, and the chain records that as the run's outcome. Migration 1055 records the pointer BEFORE the swap is performed; `publish_generation`'s docstring carries the argument for that order and names the irreducible residue (the cluster takes no part in the transaction). | Writing a real `PublicationSwap` and a real `MetastoreMetadata` are the two operator-facing preconditions left for E1/E2. |
