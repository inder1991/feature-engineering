...
| Item | Detail | Trigger |
|---|---|---|
| 🔴 **`_resolve_ratio` promises operand numericness and does not check it** | `output_authority.py::_resolve_ratio`'s docstring states "§C RATIO — **numeric both operands**", but it reads `num.output_type`/`den.output_type` only through `_needs_authority` (which tests C1 *read health*: fork / hash-mismatch / projection-unavailable) and then returns a hardcoded `decimal`. Numericness is never tested. This was recorded as a Child-1 **Minor** ("RATIO operand-numericness not enforced"); Task 8 found its **physical** consequence: a ratio over binary-float operands resolves to `decimal` and publishes `DECIMAL(38,6)` — a fixed-point column implying a reproducibility float arithmetic does not have. Same gap, now one layer down and materially worse. **Fix location:** `_resolve_ratio`, using the `_is_numeric_logical_type` helper the SUM arm already uses — the docstring already claims it happens there. Note that file is currently the parallel feature-generation stream's territory. | Before anything leaves sandbox. A published `DECIMAL` derived from float arithmetic is not reproducible run-to-run. |
| 🟡 **`resolve_physical_type(formula)` cannot see a ratio's operands** | Structural: Child-1 collapses a formula to ONE logical word (a SUM's own operand, a DIFFERENCE's *minuend*, and for a RATIO a constant describing no operand at all), so the physical adapter has nothing per-expression to inspect even if it wanted to. Closing A.9's gap at the physical layer instead would mean threading per-expression C1 facts into that signature, or moving the rule beside `compile_expression` where those facts already exist. | Whichever fix is chosen for A.9 above. |
| 🟡 **`external_type_required` is consumed by nothing** | A SUM over an ungoverned-but-numeric operand publishes fixed-point on an ungoverned read. §6 is silent and Task 8 did not invent a gate. | When an externally-unvalidated type must not reach a published column. |
