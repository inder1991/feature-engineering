...
| 🟢 **The probe's verdict is DERIVED from the observations — the DRIVER landed 2026-08-15 (D2)** | The plan makes `passed` and `covers_schema_evolution` fields of a frozen `ProbeResult`, and a field a caller sets is a caller who can assert capability — exactly the Rev-3 defect §10.3 removed from `record_attestation`. Both are re-derived in `__post_init__` and a disagreeing claim is refused, so the single-parameter `record_attestation(conn, probe_result)` is load-bearing rather than decorative. `evidence_hash` is verified the same way. **A probe that observed nothing, or whose readers only ever saw one generation, cannot report a pass at all** — the vacuity guard is in the type, not only in 16b's test. | **Met.** `materialize/probe.py`'s `probe_publication_capability` collects observations and calls `assess_probe_observations`, and `test_the_driver_states_no_verdict_of_its_own` asserts over the module's AST that `ProbeResult` is never CONSTRUCTED there — structural, not observed. Every reading is handed over including the torn ones, and a reader that could not read contributes nothing rather than a fabricated observation. **The LIVE probe against a cluster has never been run** and remains an operator action. |
...
| ⚪ **The reader-visible POINTER SWITCH is not rendered — CLOSED 2026-08-15 (D3), as a SEAM rather than as rendered DDL** | Still not rendered, and deliberately: the switch is `publish.PublicationSwap`, a Protocol the DEPLOYMENT implements, called by `publish_generation` after §9's gates. Rendering the DDL would have put the operation inside the artifact, where the probe's attestation says nothing about it. **SUCCESSOR 2 (2026-08-15) wrote the implementation**, `publish_sql.SqlPublicationSwap`, and it does not break the render-only invariant: it imports no pyspark, needs no JVM, and moves the pointer with ONE `CREATE OR REPLACE VIEW` over a DB-API 2.0 cursor to the engine's own SQL endpoint — the engine's own atomic metastore operation, which is the act the probe attests. It reads the result back afterwards, because `deploy/kind/sandbox/up.sh` records twice that a DDL statement returning without error proves nothing. `swap=None` remains the posture of a deployment that states no execution block, and the chain still records that as the run's outcome. Migration 1055 records the pointer BEFORE the swap is performed; `publish_generation`'s docstring carries the argument for that order and names the irreducible residue (the cluster takes no part in the transaction). | Both adapters are written. What E2 still needs is a SQL endpoint in front of the kind sandbox's metastore — `sandbox/40-spark.yaml` deploys a `kubectl exec`-driven pod with no Service and no thrift server — plus Task 0's inventory. |
...
| Item | Why deferred | Trigger to revisit |
|---|---|---|
| ⚪ **`requested → failed` is not a legal transition — DECIDED 2026-08-15 (D4): the edge is NOT added, and the SURFACE names the class instead** | The call §3.2 was owed, made. `GET /materialization-runs/{id}` now answers `outcome: "never_accepted"` for a `requested` request whose queue message is no longer claimable, with a `run_status_reason` that names the class, says what to check (`FEATUREGEN_MATERIALIZE_*` on the worker) and says what to do (re-trigger with a FRESH idempotency key — §3.3's rule that a re-run is a NEW request). `LEGAL_LIFECYCLE_TRANSITIONS` is unchanged and the row stays honestly non-terminal. **Three reasons the edge lost.** (1) The reconciler already REFUSES to invent a verdict for this class — `NO_LEGAL_TERMINAL`, reported and gauged — and the edge would convert that refusal into a terminalization on the evidence "the message is unreachable", which is a judgement about work nobody ever claimed. (2) The operator's actual complaint was that the status surface showed it as pending indefinitely; that is a SURFACE defect and it is closed with no state-machine change. (3) Terminalizing is a one-way door on a row whose whole value is that it is honest about never having been claimed; saying so is not, and nothing is lost while the row stands. **What is still true and now visible:** the class exists, it accumulates, and `materialize.reconcile.no_legal_terminal` still gauges it. | Reopen only if an operator needs a stuck request CLOSED rather than explained — a retention or listing problem, not a legibility one. `api/routes/materialization_runs.py::_is_stranded` carries the argument; `test_a_request_that_was_never_accepted_is_legible` pins the answer. |
