...
def propose_bridge(conn, candidate: BridgeCandidateV1, *, actor, now=None,
    ...
    # Concept authorities are per-SIDE evidence exactly like the grain flags: they travel with
    # their endpoints through canonicalization, or the evidence credits one catalog's authority
    # to the other.
    left_concept_authority, right_concept_authority = (
        (candidate.right_concept_authority, candidate.left_concept_authority) if swapped
        else (candidate.left_concept_authority, candidate.right_concept_authority))
    evidence = {"entity_id": candidate.entity_id, "candidate_id": candidate.candidate_id,
                ...
                "left_concept_authority": left_concept_authority,
                "right_concept_authority": right_concept_authority,
    ...
    # Canonical orientation on the PROPOSED value: `ref` reordered on construction, so the value
    # a fingerprint is computed over always carries ONE orientation (main's identity fix).
