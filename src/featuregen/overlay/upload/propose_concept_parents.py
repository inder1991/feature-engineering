...
# (`test_the_offline_proposals_validate_against_a_pre_patch_registry`, which clears `is_a` first so
# the validator actually runs) and they land as a reviewed source diff, which is exactly the shape
# the LLM path was designed to produce.
#
# THE RULE APPLIED: "every X is a Y" must be literally true, Y must already be in the registry, and
# where the parent declares an `additivity` the child's must agree with it. Where no existing concept
# is a true generalisation, the concept STAYS A ROOT. That is why this set does not cover all 272:
#
#   * the 49 `identifier` concepts have no abstract root to point at. NOT because one is impossible —
#     an earlier version of this comment claimed `_validate_registry` forbids it, and that was WRONG:
#     the rule is only `group == "identifier" => namespace`, nothing constrains `is_a` across groups,
#     and both an out-of-group root (`Concept("party_identifier", "categorical", ...)`) and an
#     identifier root carrying its own private namespace validate today. The real barrier is scope:
#     this mechanism may only choose parents that already exist, and ADDING a concept widens
#     `classification_vocabulary()` and therefore the enrichment classifier's prompt, which is a
#     vocabulary decision with its own blast radius rather than a backfill. Deferred, not blocked.
#     Parenting the identifiers to each other instead (`merchant_id is_a customer_id`) would assert a
...
OFFLINE_PROPOSALS: dict[str, str] = {
    ...
    # Review correction: this was first proposed as `boolean_flag` on the word "marker". Its own
    # description enumerates FOUR states (missing / imputed / stale / reconciliation-break), which is
    # a coded category, not a boolean. The rule must fire on the VALUE DOMAIN, not on a noun.
    "data_quality_flag": "category_code",
    ...
    # `potential_future_exposure` was REVERTED in review and is deliberately absent. It was proposed
    # as `monetary_stock` by analogy with `expected_exposure`, and the analogy is false:
    # `expected_exposure` is semi_additive and says "sum across netting sets", so it really is a
    # stock, while PFE is non_additive and says "never sum across netting sets, like var". Parenting
    # it to a semi_additive stock hands a model the opposite summation licence. The registry already
    # agrees with itself here — `var` and `expected_shortfall`, which PFE says it behaves like, are
    # roots. ADDITIVITY MUST AGREE WITH THE PARENT'S, and a shared word ("exposure") is not evidence.
