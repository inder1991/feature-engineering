"""VENDORED RFC 8785 (JSON Canonicalization Scheme) implementation.

Vendored VERBATIM (module docstring replaced by this attribution; no code
changes) from:

    https://github.com/trailofbits/rfc8785.py
    src/rfc8785/_impl.py @ 5cda7f4404958ff7d70dd9cbe9c83acd9273d566 (v0.1.4)
    Copyright (c) Trail of Bits -- Apache License, Version 2.0

which is itself adapted from Anders Rundgren's JCS reference implementation
(Apache-2.0), listed by RFC 8785 Appendix G as a verified-compatible
implementation:

    https://github.com/cyberphone/json-canonicalization
    python3/src/org/webpki/json/Canonicalize.py

Proven against the RFC 8785 test vectors in
``tests/featuregen/formula/test_jcs_vectors.py`` (section 3.2.4 canonical
bytes, section 3.2.3 UTF-16 key sorting, Appendix B number samples).

Public surface used by ``featuregen.formula.canonical``: ``dumps(obj) -> bytes``
(canonical UTF-8) and ``CanonicalizationError``.
"""
