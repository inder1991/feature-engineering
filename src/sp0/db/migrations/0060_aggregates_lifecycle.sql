...
CREATE INDEX IF NOT EXISTS feature_versions_feature_idx ON feature_versions (feature_id);
CREATE INDEX IF NOT EXISTS feature_versions_base_idx    ON feature_versions (base_feature_version_id);
...
CREATE OR REPLACE TRIGGER feature_versions_no_mutation
    BEFORE UPDATE OR DELETE ON feature_versions
    FOR EACH ROW EXECUTE FUNCTION feature_versions_write_once();

CREATE TABLE IF NOT EXISTS feature_active_versions (
    feature_id         text        NOT NULL,
...
CREATE INDEX IF NOT EXISTS consumers_feature_active_idx ON consumers (feature_id) WHERE edge_status = 'active';
