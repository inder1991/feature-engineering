...
const AXIS_FIELDS: readonly [string, string][] = [
  ...
  // unconfirmed, else "nothing known yet".
...
//
// PORTED onto the card design (2026-08-09). This section landed on `main` while the dossier
// restructure was being built on a branch 285 commits behind, so the restructure had no counterpart
// for it and a naive merge would have deleted a shipped feature. Its BEHAVIOUR is carried over
// unchanged — only the chrome becomes a `DossierCard`, so it sits beside the source glossary's own
// vocabulary rather than in a separate stack of equal-weight sections.
...
function SearchTermsCard({ detail }: { detail: AssetDetail }) {
  // The whole card is absent when the evidence section was not served — a subset `include=` or a
  ...
  //
  // ANY non-active bucket counts, not just `stale`: `_evidence_section` buckets by whatever
  // lifecycle the row carries and creates a bucket for any it does not know, so reading one name
  // would turn a future lifecycle into a clean "nothing was ever drafted" claim over drafts that
  // exist. Counting the complement of `active` cannot go stale that way.
...
function SemanticsCard({
  ...
  //
  // ONE NARROW EXCEPTION, and it is a merge decision worth stating (2026-08-09). `main` shipped the
  // opposite rule for `sub_domain` alone: "a deployment whose backend predates the axis must look
  // exactly like today's, not like a column whose sub-domain is missing." Both rules are right about
  // different problems — the general one is about HONESTY across established axes, main's is about
  // DEPLOYMENT SKEW for the newest one. They are only in conflict if the exception is allowed to
  // generalise, so it is keyed to the axis's ABSENCE from the payload (`in`, not null-ness): a
  // backend that sends `sub_domain: null` has an opinion and gets a row; one that never heard of the
  // axis gets no row. Every other axis keeps the always-render rule untouched.
  const visibleAxes = AXIS_FIELDS.filter(
    ([name]) => name !== 'sub_domain' || (metadata ? name in metadata.fields : false))
  const known = axesApply
    ? visibleAxes.filter(([name]) => axisIsKnown(metadata?.fields[name])).length
    : 0
  const unknown = visibleAxes.length - known
  return (
    <DossierCard
      full
