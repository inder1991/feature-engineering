  ...
  return (
    <li className="row q-item sfc">
      ...
      <div className="sfc-inputs" aria-label="Input columns and recipe roles">
        ...
        <ul>
          {s.operands.map((operand, index) => (
            <li key={`${operand.catalog_source}:${operand.graph_object_ref}:${index}`}>
              <span className="mono">{columnOf(operand.graph_object_ref)}</span>
              <small>{operand.recipe_role ?? operand.classification}</small>
            </li>
          ))}
        </ul>
      </div>
      ...
      {s.eligibility_note !== null && (
        <div className="sfc-safety-note sfc-safety-note--eligibility">
          <strong>Eligibility and leakage</strong>
          <span className="sfc-clamp">{s.eligibility_note.value}</span>
        </div>
      )}
